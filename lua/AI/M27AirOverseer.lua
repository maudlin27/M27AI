...
function ClearTrackersOnUnitsTargets(oAirUnit, bOnlyRemoveFirstEntry)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': Start of code. oAirUnit='..oAirUnit.UnitId..M27UnitInfo.GetUnitLifetimeCount(oAirUnit)..'; Is the air unit target list empty='..tostring(M27Utilities.IsTableEmpty(oAirUnit[reftTargetList]))) end
    ...
    if not (bDontIssueCommands) then
        ...
        if EntityCategoryContains(M27UnitInfo.refCategoryTorpBomber, oAirUnit.UnitId) then
            ...
            if bMicroTurn then
                ...
                if bDebugMessages == true then LOG(sFunctionRef..': Telling oAirUnit '..oAirUnit.UnitId..M27UnitInfo.GetUnitLifetimeCount(oAirUnit)..' to move to rally point '..repr(tRallyPoint)) end
    ...
    if EntityCategoryContains(refCategoryAirScout, oKilled.UnitId) == true then
        ...
        if oKiller.GetAIBrain then
            if not (M27Utilities.CanSeeUnit(aiBrain, oKiller, true)) then
                ...
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': Unit died from unseen killer, so will flag to scout it sooner. Killed position='..repr(oKilled:GetPosition()))
                end
        ...
        if M27Utilities.IsTableEmpty(aiBrain[M27MapInfo.reftRallyPoints]) then
            ...
            if iMaxDistance >= 30 then
                ...
                for iRallyPoint, tRallyPoint in aiBrain[M27MapInfo.reftRallyPoints] do
                    ...
                    if bDebugMessages == true then LOG(sFunctionRef..': tRallyPoint='..repr(tRallyPoint)..'; iCurDistanceToStart='..iCurDistanceToStart..'; iMaxDistance='..iMaxDistance..'; iFurthestFromStart='..iFurthestFromStart) end
                    if iCurDistanceToStart <= iMaxDistance and iCurDistanceToStart > iFurthestFromStart then
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': Will set the nearest rally point to iNearestRallypoint '..iNearestRallyPoint..' which is '..repr(aiBrain[M27MapInfo.reftRallyPoints][iNearestRallyPoint])) end
                    end
                end
            end
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Returning rally point '..repr(aiBrain[M27MapInfo.reftRallyPoints][iNearestRallyPoint])) end
            M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerEnd)
    ...
    for iUnit, oUnit in tPotentialUnitTargets do
        ...
        if not(oUnit == oRecentlyTargeted) and oUnit:GetFractionComplete() >= (iMinFractionComplete or 0.5) then
            ...
            if GetMaxStrikeDamageWanted(oUnit) > (oUnit[refiStrikeDamageAssigned] or 0) or oUnit == oBomberCurTarget then
                ...
                if not(oBomber[refbEngiHunterMode]) or (aiBrain[refiHighestEnemyAirThreat] <= 60 and (aiBrain[refiEnemyMassInGroundAA] <= 10 or M27UnitInfo.GetUnitLifetimeCount(oBomber) == 1)) or M27Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain)) > 80 then
                    if not(iAATechToCheckFor) or not(IsTargetCoveredByAA(oUnit, tEnemyAAToAvoid, iAATechToCheckFor, oBomber:GetPosition())) then
...
function UpdateBomberTargets(oBomber, bRemoveIfOnLand, bLookForHigherPriorityShortlist, bReissueIfBlocked)
    ...
    if M27UnitInfo.IsUnitValid(oBomber) then
        if oBomber[refbEngiHunterMode] then
            ...
            if M27UnitInfo.IsUnitValid(oBomberCurTarget) and M27Utilities.GetDistanceBetweenPositions(oBomberCurTarget:GetPosition(), oBomber:GetPosition()) <= 110 then
                if math.abs(M27UnitInfo.GetUnitFacingAngle(oBomber) - M27Utilities.GetAngleFromAToB(oBomber:GetPosition(), oBomberCurTarget:GetPosition())) <= 20 then
            ...
            if not(bKeepExistingTarget) then
                ...
                elseif not (oTargetWanted) and not(oBomberCurTarget) then
                    ...
                    if bDebugMessages == true then LOG(sFunctionRef..': Dont have a target to select, and dont ahve an existing target, so will consider moving somewhere. bWantNewMovementTarget='..tostring(bWantNewMovementTarget)..'; tCurTarget='..repr((tCurTarget or {'nil'}))..'; Bomber unit state='..M27Logic.GetUnitState(oBomber)) end
                    if bWantNewMovementTarget then
                                ...
                                if M27Utilities.GetDistanceBetweenPositions(tMex, tOurBase) > iDistToEnemyBase then
                                    if iDistToEnemyBase >= 100 or (aiBrain[refiHighestEnemyAirThreat] <= 60 and (aiBrain[refiEnemyMassInGroundAA] <= 10 or M27UnitInfo.GetUnitLifetimeCount(oBomber) == 1)) then
                                        ...
                                        if iCurDistance >= 30 and iCurDistance < iLowestDistance then
                                            --Is there any AA covering it?
                                            if not(IsTargetPositionCoveredByAA(tMex, tEnemyAA, oBomber:GetPosition(), false)) then
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': Just given order to oBomber='..oBomber.UnitId..M27UnitInfo.GetUnitLifetimeCount(oBomber)..' to move to target '..repr(tNewTarget)..'; bAttackMove='..tostring(bAttackMove)) end
                        ...
                        while bBomberHasDeadOrShieldedTarget == true do
                            ...
                            elseif bRemoveIfOnLand then
                                ...
                                if iDeadLoopCount == 1 and oBomber.GetNavigator and not(oBomber[M27UnitInfo.refbSpecialMicroActive]) and not(M27Utilities.IsTableEmpty(oBomber[reftGroundAttackLocation])) then
                                    ...
                                    if bDebugMessages == true then LOG(sFunctionRef..': oBomber[reftGroundAttackLocation]='..repr(oBomber[reftGroundAttackLocation])..'; distance to the bomber target='..M27Utilities.GetDistanceBetweenPositions(oBomber[reftGroundAttackLocation], oBomberCurTarget:GetPosition())..'; iAOE='..iAOE) end
                                ...
                                if iCategoriesToSearch and not (EntityCategoryContains(iCategoriesToSearch, oBomberCurTarget.UnitId)) then
                                    ...
                                    if iNearbyPriorityShieldTargets + iNearbyPriorityAATargets > 0 then
                                        ...
                                        if bChangeTarget then
                                            ...
                                            if bDebugMessages == true then
                                                LOG(sFunctionRef .. ': Want to switch target to high priority AA or shield')
                                            end
                                            ClearAirUnitAssignmentTrackers(aiBrain, oBomber, true)
                            ...
                            if bRemoveCurTarget == true and not (bHaveAssignedNewTarget) then
                                ...
                                if bDebugMessages == true then
                                    LOG(sFunctionRef .. ': Removing bombers current target as it is nil or dead, iCurTargetNumber=' .. oBomber[refiCurTargetNumber])
                                end
                                ...
                                if bReissueIfBlocked then
                                    ...
                                    if bDebugMessages == true then
                                        LOG(sFunctionRef .. 'Recently fired bomb, checking if we think we are blocked when attacking target; tPreTargetViaPoint=' .. repr((tPreTargetViaPoint or { 'nil' })))
                                    end
                                ...
                                if bDebugMessages == true then
                                    LOG(sFunctionRef .. ': Bomber current target isnt dead, UnitId=' .. oBomberCurTarget.UnitId .. '; will draw circle around target position ' .. repr(oBomberCurTarget:GetPosition()))
        ...
        if M27UnitInfo.IsUnitValid(oTarget) then
            ...
            if not(bClearTarget) then
                ...
                if oBomber.GetNavigator and not(oBomber[M27UnitInfo.refbSpecialMicroActive]) and not(M27Utilities.IsTableEmpty(oBomber[reftGroundAttackLocation])) then
                    local iAOE, iStrikeDamage = M27UnitInfo.GetBomberAOEAndStrikeDamage(oBomber)
                    if bDebugMessages == true then LOG(sFunctionRef..': oBomber[reftGroundAttackLocation]='..repr((oBomber[reftGroundAttackLocation] or {'nil'}))..'; distance to the bomber target='..M27Utilities.GetDistanceBetweenPositions((oBomber[reftGroundAttackLocation] or {0,0,0}), oTarget:GetPosition())..'; iAOE='..(iAOE or 'nil')) end
...
function AirThreatChecker(aiBrain)
    ...
    if M27Utilities.IsTableEmpty(tEnemyAirUnits) == false then
        ...
        if oNearestAirUnit then aiBrain[reftNearestEnemyAirThreat] = oNearestAirUnit:GetPosition() end
    end
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': About to calcualte threat level of enemy antiair units')
    end
    ...
    local iAirAAWantedBasedOnThreat = math.max(0, ((aiBrain[refiHighestEnemyAirThreat] or 0) - (aiBrain[refiOurMassInAirAA] or 0)) / tiAirAAThreatByTech[math.min(3, aiBrain[M27Overseer.refiOurHighestAirFactoryTech])])
                            ...
                            if oUnit[refbOnAssignment] == true then
                                    ...
                                    if not(oUnit[M27UnitInfo.refbSpecialMicroActive]) then
                                        if iUnitType == iTypeScout then
                                            ...
                                            if not (oUnit[refoAssignedAirScout]) then
                                                ...
                                                if bDebugMessages == true then
                                                    LOG(sFunctionRef .. ': Checking if iUnit=' .. iUnit .. ' has reached its target; cur target=' .. repr(tCurWaypointTarget))
                                                end
                                                if M27Utilities.IsTableEmpty(tCurWaypointTarget) == false then
                                                    if bDebugMessages == true then
                                                        LOG(sFunctionRef .. ': tCurWaypointTarget=' .. repr(tCurWaypointTarget))
                                                    end
                                                    ...
                                                    while iDistanceToCurTarget <= iDistanceToComplete or iTimeStamp - aiBrain[reftAirSegmentTracker][iCurAirSegmentX][iCurAirSegmentZ][refiLastScouted] <= math.max(2, aiBrain[reftAirSegmentTracker][iCurAirSegmentX][iCurAirSegmentZ][refiCurrentScoutingInterval] - 10) do
                                                        ...
                                                        if oUnit[refiCurMovementPath] > iTotalMovementPaths then
                                                            ...
                                                            tCurWaypointTarget = oUnit[reftMovementPath][oUnit[refiCurMovementPath]]
                                                            if bDebugMessages == true then
                                                                LOG(sFunctionRef .. ': Scout has reached target, increasing its movement path 1 to ' .. oUnit[refiCurMovementPath] .. '; location of this=' .. repr(tCurWaypointTarget))
                                                            end
                                ...
                                if oUnit[M27UnitInfo.refbSpecialMicroActive] then
                                    ...
                                    if not (oUnit[refbSentRefuelCommand]) then
                                        ...
                                        if iUnitType == iTypeBomber and M27UnitInfo.GetUnitLifetimeCount(oUnit) <= 2 and iCurTechLevel == 1 then
...
function UpdateScoutingSegmentRequirements(aiBrain)
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': Start of code, Nearest enemy start point=' .. repr(M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain)) .. '; aiBrain[refiMinSegmentX]=' .. aiBrain[refiMinSegmentX] .. '; aiBrain[refiMaxSegmentX]=' .. aiBrain[refiMaxSegmentX] .. '; aiBrain[refiMinSegmentX]=' .. aiBrain[refiMinSegmentX] .. '; aiBrain[refiMaxSegmentZ]=' .. aiBrain[refiMaxSegmentZ])
    end
...
function GetEndDestinationForScout(aiBrain, oScout)
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': Start of code, Shortlist size=' .. table.getn(aiBrain[reftScoutingTargetShortlist]) .. '; reftScoutingTargetShortlist=' .. repr(aiBrain[reftScoutingTargetShortlist]) .. '; Enemy start position=' .. repr(M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain)))
    end
    ...
    if iFinalCount then
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': Final destination XZ segments=' .. iFinalX .. '-' .. iFinalZ .. '; about to check for via points')
        end
...
function CreateMovementPathFromDestination(aiBrain, tEndDestination, oScout)
    ...
    if aiBrain[reftAirSegmentTracker][iEndDestinationX][iEndDestinationZ][refiDeadScoutsSinceLastReveal] > 0 then
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': Orig end point was ' .. repr(aiBrain[reftAirSegmentTracker][iEndDestinationX][iEndDestinationZ][reftMidpointPosition]) .. '; new end point is ' .. repr(tEndViaPoint))
        end
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': End of code, tMovementPath=' .. repr(tMovementPath))
    end
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerEnd)
    ...
    if M27Utilities.IsTableEmpty(aiBrain[reftAvailableScouts]) == false then
        ...
        for iUnit, oUnit in aiBrain[reftAvailableScouts] do
            if M27Utilities.IsTableEmpty(tDedicatedScoutAssistTargets) == false then
                ...
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': iUnit=' .. iUnit .. ': End destination=' .. repr(tEndDestination))
                end
                if M27Utilities.IsTableEmpty(tEndDestination) == false then
                    ...
                    for iWaypoint, tWaypoint in tMovementPath do
                        ...
                        if bDebugMessages == true then
                            LOG(sFunctionRef .. ': Issuing move command to go to Segment X-Z=' .. iCurAirSegmentX .. iCurAirSegmentZ .. '; destination=' .. repr(tWaypoint))
                        end
...
function GetVulnerableMexes(aiBrain, tStartPoint, iShieldHealthToIgnore)
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': tStartPoint=' .. repr(tStartPoint) .. '; iShieldHealthToignore=' .. iShieldHealthToIgnore .. '; Size of tEnemyMexes=' .. table.getn(tEnemyMexes))
    end
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': End of code, tVulnerableMexes=' .. repr((tVulnerableMexes or { 'nil' })))
    end
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerEnd)
...
function GetBomberTargetShortlist(aiBrain)
    ...
    local iEnemyAASearchRange = 40 + 20 * aiBrain[M27Overseer.refiEnemyHighestTechLevel]
    ...
    if M27Utilities.IsTableEmpty(aiBrain[M27Overseer.reftEnemyLandExperimentals]) == false then
        ...
        if iClosestEnemyExperimental <= 275 then
            aiBrain[refiBomberDefencePercentRange] = math.max(aiBrain[refiBomberDefencePercentRange], iClosestEnemyExperimental / aiBrain[M27Overseer.refiDistanceToNearestEnemyBase])
        end
    end
    ...
    if M27Utilities.IsTableEmpty(tOurHighValueBuildings) == false then
        ...
        for iUnit, oUnit in tOurHighValueBuildings do
            if oUnit:GetFractionComplete() < 1 or EntityCategoryContains(categories.STRUCTURE, oUnit.UnitId) then
                iCurDistance = M27Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber])
                if iCurDistance > iFurthestFriendlyHighValueBuilding then
                    iFurthestFriendlyHighValueBuilding = iCurDistance
                end
            end
        end
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': aiBrain[refiBomberDefencePercentRange]=' .. aiBrain[refiBomberDefencePercentRange] .. '; Numerically this is ' .. aiBrain[refiBomberDefencePercentRange] * aiBrain[M27Overseer.refiDistanceToNearestEnemyBase])
        end
    end
    if aiBrain[refiBomberDefencePercentRange] > 350 then
        aiBrain[refiBomberDefencePercentRange] = 350
    end
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': Deciding what categories and priorities to look for')
    end
    if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyACUKill and M27UnitInfo.IsUnitValid(aiBrain[M27Overseer.refoLastNearestACU]) and M27UnitInfo.IsUnitUnderwater(aiBrain[M27Overseer.refoLastNearestACU]) == false then
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': Want to target enemy ACU only')
        end
    ...
    elseif aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyAirDominance then
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': In air dominance mode so focus down any ground AA')
        end
    else
        ...
        if aiBrain[M27Overseer.refiPercentageOutstandingThreat] <= aiBrain[refiBomberDefencePercentRange] or aiBrain[M27Overseer.refiModDistFromStartNearestOutstandingThreat] <= 150 or aiBrain[M27Overseer.refiModDistFromStartNearestThreat] <= (aiBrain[refiBomberDefencePercentRange] * aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] - 50) then
            ...
            iMaxLifetimeMassMod = 5
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': Enemies near base so focus down AA first then land; iMaxSearchRange to use=' .. iMaxSearchRange .. '; aiBrain[refiBomberDefencePercentRange]=' .. aiBrain[refiBomberDefencePercentRange])
            end
        else
            if aiBrain[M27Overseer.refiOurHighestAirFactoryTech] >= 2 then
                --Currently T2 bombers will have same target prioritisation as a T3 bomber
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': Have a T3 air factory so look for targets for a strat bomber')
                end
                ...
                if M27Utilities.IsTableEmpty(aiBrain[M27Overseer.reftEnemyLandExperimentals]) == false then
                    iLandExperimentalCat = iLandExperimentalCat + M27UnitInfo.refCategoryMAA * categories.TECH3
                end
                ...
                if GetGameTimeSeconds() <= 360 and M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryBomber, 4) then
                    ...
                    if aiBrain[M27Overseer.refiModDistFromStartNearestThreat] <= 200 then
                        --Include MAA in priority targets
                        reftPriorityTargetCategories = { M27UnitInfo.refCategoryEngineer + M27UnitInfo.refCategoryMAA + M27UnitInfo.refCategoryNavalSurface + M27UnitInfo.refCategoryLandCombat + M27UnitInfo.refCategoryRadar - categories.COMMAND - M27UnitInfo.refCategoryStructureAA, M27UnitInfo.refCategoryStructure - M27UnitInfo.refCategoryStructureAA, categories.COMMAND }
                    else
                        reftPriorityTargetCategories = { M27UnitInfo.refCategoryEngineer + M27UnitInfo.refCategoryNavalSurface + M27UnitInfo.refCategoryLandCombat + M27UnitInfo.refCategoryRadar - categories.COMMAND - M27UnitInfo.refCategoryStructureAA, M27UnitInfo.refCategoryStructure - M27UnitInfo.refCategoryStructureAA, categories.COMMAND }
                    end
                    aiBrain[refiLowPriorityStart] = 2
                end

                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': Air fac at t1, will target enemy engis')
                end
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': About to identify targets for shortlist, aiBrain[refiLowPriorityStart]=' .. aiBrain[refiLowPriorityStart])
    end
    ...
    if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyACUKill or aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyProtectACU then
        ...
        if aiBrain[M27Overseer.refiPercentageOutstandingThreat] <= aiBrain[refiBomberDefencePercentRange] or aiBrain[M27Overseer.refiModDistFromStartNearestOutstandingThreat] <= 150 then
            ...
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': Will cycle through low priority bombers to increase the shield ignore value')
            end
            if M27Utilities.IsTableEmpty(aiBrain[reftBombersAssignedLowPriorityTargets]) == false then
                for iBomber, oBomber in aiBrain[reftBombersAssignedLowPriorityTargets] do
                    if M27UnitInfo.IsUnitValid(oBomber) then
                        ...
                        if bDebugMessages == true then
                            LOG(sFunctionRef .. ': Adding bomber to the shield ignore value; oBomber=' .. oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber) .. '; aiBrain[refiShieldIgnoreValue]=' .. aiBrain[refiShieldIgnoreValue])
                        end
                    end
                end
            elseif bDebugMessages == true then
                LOG(sFunctionRef .. ': No bombers are assigned low priority targets')
            end
        end
    end
    if M27MapInfo.bNoRushActive then
        iMaxSearchRange = math.min(iMaxSearchRange, M27MapInfo.iNoRushRange)
    end

    if bDebugMessages == true then
        LOG(sFunctionRef .. ': About to cycle through different types of categories with iMaxSearchRange=' .. iMaxSearchRange .. '; aiBrain[refiShieldIgnoreValue]=' .. aiBrain[refiShieldIgnoreValue])
    end
    ...
    for iTypePriority, iCategory in reftPriorityTargetCategories do
        ...
        if iTypePriority >= aiBrain[refiLowPriorityStart] then
            if iTargetShortlistCount > 0 then
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': Already have targets in shortlist and are at a low priority category so wont proceed')
                end
                aiBrain[refbShortlistContainsLowPriorityTargets] = false
                if iTargetShortlistCount >= aiBrain[refiPreviousAvailableBombers] then
                    bProceed = false
                end
            else
                ...
                if GetGameTimeSeconds() - aiBrain[reftAirSegmentTracker][iCurX][iCurZ][refiLastScouted] > 30 and aiBrain[M27Overseer.refiOurHighestAirFactoryTech] < 3 then
                    bProceed = false
                end
                if bProceed then
                    if bDebugMessages == true then
                        LOG(sFunctionRef .. ': Dont have targets in shortlist so will considerl ow priority category; iHardToHitTargets=' .. iHardToHitTargets)
                    end
                    ...
                    if iHardToHitTargets > 0 then
                        ...
                        if bDebugMessages == true then
                            LOG(sFunctionRef .. ': Have a hard to hit target already, since are onto low priority categories will just add this to the shortlist')
                        end
                    end
                end
            end
        ...
        if bProceed == true then
            ...
            if M27Utilities.IsTableEmpty(tEnemyUnitsOfType) == false then
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': About to cycle through all units of type, size of table=' .. table.getn(tEnemyUnitsOfType))
                end
                for iUnit, oUnit in tEnemyUnitsOfType do
                    ...
                    if not (oUnit.Dead) and oUnit.GetPosition then
                        if bOnlyIncludeTargetACU then
                            if bDebugMessages == true then
                                LOG(sFunctionRef .. ': Only want to target the ACU, checking if this unit is the last nearest ACU')
                            end
                            ...
                            if iMaxPercentDistance >= 1 or iMaxPercentDistance >= (iCurDistanceToBase / (iCurDistanceToBase + iCurDistanceToEnemy)) then
                                --Ignore if its a T1 mex or air or land factory and close to enemy base unless as a low priority target
                                if aiBrain[refbShortlistContainsLowPriorityTargets] or iCurDistanceToEnemy >= 160 or not (EntityCategoryContains(M27UnitInfo.refCategoryT1Mex + M27UnitInfo.refCategoryLandFactory + M27UnitInfo.refCategoryAirFactory, oUnit.UnitId)) then
                                    ...
                                    if oUnit[refiCurBombersAssigned] == nil or oUnit[refiCurBombersAssigned] == 0 then
                                        bIncludeInShortlist = true
                                    elseif (oUnit[refiStrikeDamageAssigned] or 0) < iMaxUnitStrikeDamageWanted then
                                        bIncludeInShortlist = true
                                    end
                                    ...
                                    if bIncludeInShortlist == true then
                                        if (oUnit[refiLifetimeFailedBombersAssigned] or 0) >= iMaxLifetimeAssignment and oUnit[refiLifetimeFailedBomberMassAssigned] > oUnit:GetBlueprint().Economy.BuildCostMass * iMaxLifetimeMassMod then
                                            if bDebugMessages == true then
                                                LOG(sFunctionRef .. ': Have already assigned ' .. oUnit[refiLifetimeFailedBombersAssigned] .. ' bombers with mass value of ' .. (oUnit[refiLifetimeFailedBomberMassAssigned] or 'nil') .. ' to a unit with a mass cost of ' .. oUnit:GetBlueprint().Economy.BuildCostMass .. ' that have died so wont assign any more except as a low priority target')
                                            end
                                        ...
                                        if M27UnitInfo.IsUnitUnderwater(oUnit) then
                                            if bDebugMessages == true then
                                                LOG(sFunctionRef .. ': Unit is underwater so wont include')
                                            end
                                            bIncludeInShortlist = false
                                        end
                                        if bIncludeInShortlist == true then
                                            ...
                                            if EntityCategoryContains(M27UnitInfo.refCategoryGroundAA, oUnit.UnitId) then
                                                ...
                                                if bDebugMessages == true then
                                                    LOG(sFunctionRef .. ': Are targetting AA so wont include check about nearby AA')
                                                end
                                            else
                                                tEnemyGroundAA = aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryGroundAA, oUnit:GetPosition(), iEnemyAASearchRange, 'Enemy')
                                                if bDebugMessages == true then
                                                    LOG(sFunctionRef .. ': iTypePriority=' .. iTypePriority .. '; iUnit=' .. iUnit .. '; size of tEnemyGroundAA=' .. table.getn(tEnemyGroundAA) .. '; iEnemyAASearchRange=' .. iEnemyAASearchRange)
                                                end
                                                if M27Utilities.IsTableEmpty(tEnemyGroundAA) == true then
                                                    ...
                                                    if bDebugMessages == true then
                                                        LOG(sFunctionRef .. ': aiBrain[M27Overseer.refiOurHighestAirFactoryTech]=' .. aiBrain[M27Overseer.refiOurHighestAirFactoryTech] .. '; if this is >=3 then will ignore low level ground AA')
                                                    end
                                                    if aiBrain[M27Overseer.refiOurHighestAirFactoryTech] >= 3 then
                                                        ...
                                                        if M27Utilities.IsTableEmpty(tEnemyT3OrCruiserGroundAA) == true then
                                                            ...
                                                            if bDebugMessages == true then
                                                                LOG(sFunctionRef .. ': T3 anti air near target')
                                                            end
                                                        end
                                            ...
                                            if bAAAroundTarget == false then
                                                ...
                                                if M27Logic.IsTargetUnderShield(aiBrain, oUnit, aiBrain[refiShieldIgnoreValue]) == false then
                                                    ...
                                                    if EntityCategoryContains(M27UnitInfo.refCategoryMex, oUnit.UnitId) then
                                                        iFriendlyUnitActualSearchRange = iFriendlyUnitMexSearchRange
                                                    else
                                                        iFriendlyUnitActualSearchRange = iFriendlyUnitNormalSearchRange
                                                    end

                                                    if bDebugMessages == true then
                                                        LOG(sFunctionRef .. ': oUnit:GetPosition()=' .. repr(oUnit:GetPosition()) .. '; iFriendlyUnitActualSearchRange=' .. iFriendlyUnitActualSearchRange)
                                                    end
                                                    ...
                                                    if M27Utilities.IsTableEmpty(tFriendlyUnitsNearMex) == false then
                                                        if bDebugMessages == true then
                                                            LOG(sFunctionRef .. ': Have units near target already')
                                                        end
                                                        bIncludeInShortlist = false
                                                    elseif bDebugMessages == true then
                                                        LOG(sFunctionRef .. ': Target has no nearby units so will keep in shortlist')
                                                    end
                                                    --end
                                                else
                                                    bIncludeInShortlist = false
                                                ...
                                                bIncludeInShortlist = false
                                                if bDebugMessages == true then
                                                    LOG(sFunctionRef .. ': Target has AA near it so wont target')
                                                end
                                            end
                                        end
                                    end
                                elseif bDebugMessages == true then
                                    LOG(sFunctionRef .. ': oUnit=' .. oUnit.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oUnit) .. '; iCurDistanceToEnemy=' .. (iCurDistanceToEnemy or 'nil') .. '; is a mex/factory too close to enemy base')
                                end
                            end
                        end
                        if bIncludeInShortlist == true then
                            --Is it hard to hit?
                            if not (bOnlyIncludeAsLowPriorityThreat) and (oUnit[refiFailedHitCount] < 2 or (oUnit[refiFailedHitCount] < 4 and EntityCategoryContains(M27UnitInfo.refCategoryMAA, oUnit.UnitId))) then
                                if bDebugMessages == true then
                                    LOG(sFunctionRef .. ': iTypePriority=' .. iTypePriority .. '; iUnit=' .. iUnit .. ': Including in target shortlist')
                                end
                                iTargetShortlistCount = iTargetShortlistCount + 1
                                aiBrain[reftBomberTargetShortlist][iTargetShortlistCount] = {}
                                if aiBrain[refbShortlistContainsLowPriorityTargets] then
                                    ...
                                    aiBrain[reftBomberTargetShortlist][iTargetShortlistCount][refiShortlistPriority] = iTypePriority
                                end
                                aiBrain[reftBomberTargetShortlist][iTargetShortlistCount][refiShortlistUnit] = oUnit
                            else
                                if bDebugMessages == true then
                                    LOG(sFunctionRef .. ': Unit has had ' .. (oUnit[refiFailedHitCount] or 0) .. ' failed bombs launched at it so treating as low priority')
                                end
                    ...
                    if iTargetShortlistCount > 0 and aiBrain[refbShortlistContainsLowPriorityTargets] then
                        if bDebugMessages == true then
                            LOG(sFunctionRef .. ': Are dealing with low priority targets and have a target so will abort loop')
                        end
                        break
                    end
                end
            elseif bDebugMessages == true then
                ...
                if iCategory == categories.COMMAND then
                    ...
                    if M27UnitInfo.IsUnitValid(aiBrain[M27Overseer.refoLastNearestACU]) then
                        LOG('Distance to last nearest ACU=' .. M27Utilities.GetDistanceBetweenPositions(aiBrain[M27Overseer.refoLastNearestACU]:GetPosition(), M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber]))
                    end
                end
            end
        end
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': end of loop for iTypePriority=' .. iTypePriority .. '; iTargetShortlistCount=' .. iTargetShortlistCount)
        end
    end
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerEnd)
end
...
function IssueLargeBomberAttack(aiBrain, tBombers)
    ...
    if M27Utilities.IsTableEmpty(tBombers) == true then
        ...
        if aiBrain[M27Overseer.refiOurHighestAirFactoryTech] <= 2 then
            tCategoryGroupings = { M27UnitInfo.refCategoryStructureAA, M27UnitInfo.refCategoryMAA, M27UnitInfo.refCategoryPower, M27UnitInfo.refCategoryMex, M27UnitInfo.refCategoryStructure, categories.COMMAND, categories.LAND }
        else
            tCategoryGroupings = { categories.COMMAND, categories.TECH3, categories.EXPERIMENTAL, M27UnitInfo.refCategoryStructureAA, M27UnitInfo.refCategoryMAA, M27UnitInfo.refCategoryPower, M27UnitInfo.refCategoryMex, M27UnitInfo.refCategoryStructure, categories.COMMAND, categories.LAND }
        end
        ...
        for iBomber, oBomber in tBombers do
            oBomber[refbPartOfSpecialAttack] = true
        end
        ...
        while bValidAttackPlatoon do
            ...
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': iCurLoop=' .. iCurLoop .. ': About to cycle through all bombers and get nearest one to enemy base. Total bombers=' .. table.getn(tBombers))
            end
            for iBomber, oBomber in tBombers do
                if not (oBomber.Dead) then
                    ...
                    if oBomber[refiCurTargetNumber] == nil or oBomber[refiCurTargetNumber] == 0 then
                        ...
                        if bDebugMessages == true then
                            LOG(sFunctionRef .. ': iBomber=' .. iBomber .. ': Bomber already has a current target number=' .. oBomber[refiCurTargetNumber])
                        end
                    end
                end
            end
            ...
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': Finished going through bombers, iBombersNeedingTargets=' .. iBombersNeedingTargets)
            end
            if iBombersNeedingTargets > 0 and not (M27MapInfo.bNoRushActive) then
                ...
                for iEntry, tSubvalue in M27Utilities.SortTableBySubtable(tBombersNeedingTargetsTracker, refiTrackerDistanceToEnemy, true) do
                    ...
                    for iCount, iCurCategoryCondition in tCategoryGroupings do
                        ...
                        if iCurCategoryCondition == M27UnitInfo.refCategoryStructureAA or iCurCategoryCondition == M27UnitInfo.refCategoryMAA then
                            iHealthFactorWanted = 1.5
                        end
                        ...
                        if M27Utilities.IsTableEmpty(tEnemyUnitsOfType) == false then
                            ...
                            for iEntry2, tSubvalue2 in M27Utilities.SortTableBySubtable(tEnemyUnitsToTargetTracker, refiTrackerDistanceToEnemy, true) do
                                ...
                                if tSubvalue2[refoTrackerUnit][refiStrikeDamageAssigned] == nil then
                                    bHaveCurTarget = true
                                elseif iCurTargetRemainingHealth * 1.25 > tSubvalue2[refoTrackerUnit][refiStrikeDamageAssigned] then
                                    --Want a bit of leeway incase bomber shot down or unit regens
                                    bHaveCurTarget = true
                                end
                                if bDebugMessages == true then
                                    local iStrikeDamageAssigned = tSubvalue2[refoTrackerUnit][refiStrikeDamageAssigned]
                                    if iStrikeDamageAssigned == nil then
                                        iStrikeDamageAssigned = 'nil'
                                    end
                                ...
                                if bHaveCurTarget == true then
                                    ...
                                    if bDebugMessages == true then
                                        LOG(sFunctionRef .. ': Have oCurTarget=' .. oCurTarget.UnitId)
                                    end
                                    break
                                end
                            end
                    ...
                    if bHaveCurTarget == true then
                        if oCurTarget == nil then
                            M27Utilities.ErrorHandler('Have a nil CurTarget for large attack bomber')
                        else
                            if bDebugMessages == true then
                                LOG(sFunctionRef .. ': Have a target to assign')
                            end
                        ...
                        if bDebugMessages == true then
                            LOG(sFunctionRef .. ': Dont have a current target for bomber')
                        end
                    end
                    --end
                end
...
function GetBomberPreTargetViaPoint(oBomber, tGroundTarget, bTargetingMobileUnit)
    ...
    if M27Utilities.GetDistanceBetweenPositions(oBomber:GetPosition(), tGroundTarget) > iDistanceWanted or M27UnitInfo.GetUnitTechLevel(oBomber) >= 3 then
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': Via point if dont limit to map area=' .. repr(M27Utilities.MoveInDirection(tGroundTarget, iAngleFromTargetToBomber, iDistanceWanted)) .. '; via point if limit to map area=' .. repr(M27Utilities.MoveInDirection(tGroundTarget, iAngleFromTargetToBomber, iDistanceWanted, true)))
        end
        ...
        if GetSurfaceHeight(tPossibleViaPoint[1], tPossibleViaPoint[3]) >= tPossibleViaPoint[2] or M27Logic.IsLineBlocked(aiBrain, tPossibleViaPoint, tGroundTarget, iAOE) then
            ...
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': Ground target=' .. repr(tGroundTarget) .. '; initial expected position=' .. repr(tPossibleViaPoint) .. '; line is blocked so will try alternative positions')
            end
...
function TellBomberToAttackTarget(oBomber, oTarget, bClearCommands, bAreHoverBombing)
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': tPreTargetViaPoint=' .. repr((tPreTargetViaPoint or { 'nil' })))
    end
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': Just issued attack at oTarget=' .. oTarget.UnitId .. '; Size of bomber target list=' .. table.getn(oBomber[reftTargetList]) .. '; iCurTargetNumber=' .. oBomber[refiCurTargetNumber] .. ': Issued attack order for target with position=' .. repr(tGroundTarget) .. ' and unitId=' .. oTarget.UnitId..'; GameTime='..GetGameTimeSeconds())
    end
...
function DetermineBomberDefenceRange(aiBrain)
    ...
    local iMaxRange = math.min(350, aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] * 0.5)
    ...
    --Limit reduction to 2 per second
    aiBrain[refiBomberDefenceModDistance] = math.max(aiBrain[refiBomberDefenceModDistance], iPrevDefenceRange - 2)
    if bDebugMessages == true then LOG(sFunctionRef..': Range after limiting to a reduction of 2 per second='..aiBrain[refiBomberDefenceModDistance]) end
                ...
                if oUnit:GetFractionComplete() >= 0.8 then
                    ...
                    if iCurDist < iClosestDist then
                        if EntityCategoryContains(categories.TECH3, oUnit.UnitId) or M27Logic.IsTargetUnderShield(aiBrain, oUnit, 3250, false, true, false) then
                            ...
                            if EntityCategoryContains(categories.TECH1, oUnit.UnitId) then iRange = 30 elseif EntityCategoryContains(categories.TECH2, oUnit.UnitId) then iRange = 50 else iRange = 60 end
                        end
                    end
                end
            end
...
function IsTargetPositionCoveredByAA(tTarget, tEnemyAA, tStartPoint, bReturnNumberOfAA)
    local bDebugMessages = false if M27Utilities.bGlobalDebugOverride == true then   bDebugMessages = true end
    local sFunctionRef = 'IsTargetPositionCoveredByAA'
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerStart)


    if bDebugMessages == true then LOG(sFunctionRef..': Start of code. tTarget='..repr(tTarget)..'; tStartPoint='..repr(tStartPoint)) end
    ...
    local iAARange
    if bDebugMessages == true then LOG(sFunctionRef..': Rechecking if any AA blocking. iAngleFromStartToTarget='..iAngleFromStartToTarget..'; iDistanceFromStartToTarget='..iDistFromStartToTarget) end
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': End of code, iAAInRange='..iAAInRange) end
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerEnd)
    if bReturnNumberOfAA then return iAAInRange
    else
        if iAAInRange > 0 then return true else return false end
    end
end
function IsTargetCoveredByAA(oTarget, tEnemyAA, iTechLevelOfEnemyAA, tStartPoint, bReturnNumberOfAA)
    local bDebugMessages = false if M27Utilities.bGlobalDebugOverride == true then   bDebugMessages = true end
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': Start of code. oTarget='..oTarget.UnitId..M27UnitInfo.GetUnitLifetimeCount(oTarget)..'; iTechLevelOfEnemyAA='..iTechLevelOfEnemyAA..'; tStartPoint='..repr(tStartPoint)) end
    ...
    if not(bReturnNumberOfAA) then
        if oTarget[refbLastCoveredByAAByTech][iTechLevelOfEnemyAA] and GetGameTimeSeconds() - oTarget[refiLastCoveredByAAByTech][iTechLevelOfEnemyAA] <= 10 then
            if bDebugMessages == true then LOG(sFunctionRef..': We have checked in the last 10 seconds and the target was protected. Time last checked='..GetGameTimeSeconds() - oTarget[refiLastCoveredByAAByTech][iTechLevelOfEnemyAA]) end
            M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerEnd)
            return true
        elseif oTarget[refiLastCoveredByAAByTech][iTechLevelOfEnemyAA] and GetGameTimeSeconds() - oTarget[refiLastCoveredByAAByTech][iTechLevelOfEnemyAA] <= 2 then
            if bDebugMessages == true then LOG(sFunctionRef..': We have checked in the last 2 seconds so will rely on this result.  Tiem since last checked='..GetGameTimeSeconds() - oTarget[refiLastCoveredByAAByTech][iTechLevelOfEnemyAA]) end
            M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerEnd)
            return false
        end
    end

    --Havent checked recently or want a total number count so recheck
    local iAAInRange = 0
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': End of code, about to return '..tostring(oTarget[refbLastCoveredByAAByTech][iTechLevelOfEnemyAA])..'; or iAAInRange='..iAAInRange) end
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerEnd)
    if bReturnNumberOfAA then return iAAInRange
    else
...
function AirBomberManager(aiBrain)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': Start of code, have determined bomber defence range='..aiBrain[refiBomberDefenceModDistance]..'; is table of available bombers empty='..tostring(M27Utilities.IsTableEmpty(aiBrain[reftAvailableBombers]))) end
    ...
    if M27Utilities.IsTableEmpty(aiBrain[reftAvailableBombers]) == false then
        ...
        if iNearestCruiserModDistance <= aiBrain[refiBomberDefenceModDistance] + 30 then bAvoidCruisers = true end
        ...
        function AddUnitToShortlist(oUnit, iTechLevel, iOptionalModDistanceToBase, iOptionalMaxStrikeDamageWanted)
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Will add oUnit='..oUnit.UnitId..M27UnitInfo.GetUnitLifetimeCount(oUnit)..' to shortlist unless we have assigned enough strike damage.  oUnit[refiMaxStrikeDamageWanted]='..(oUnit[refiMaxStrikeDamageWanted] or 0)..'; oUnit[refiStrikeDamageAssigned]='..(oUnit[refiStrikeDamageAssigned] or 0)) end

            if oUnit[refiStrikeDamageAssigned] < oUnit[refiMaxStrikeDamageWanted] or EntityCategoryContains(categories.COMMAND + categories.EXPERIMENTAL, oUnit.UnitId) then
            ...
            elseif bDebugMessages == true then LOG(sFunctionRef..': Already assigned enough strike damage')
            end
        end
        ...
        for iTechLevel = 1, 4 do
            ...
            iAvailableBombers = 0
            if bDebugMessages == true then LOG(sFunctionRef..': Is table of bombers of iTechLevel='..iTechLevel..' empty='..tostring(M27Utilities.IsTableEmpty(tBombersOfTechLevel))..'; cur strategy='..aiBrain[M27Overseer.refiAIBrainCurrentStrategy]..'; GameTime='..GetGameTimeSeconds()..'; iMaxPossibleRange for bomber defence='..iMaxPossibleRange) end
            if M27Utilities.IsTableEmpty(tBombersOfTechLevel) == false then
                iAvailableBombers = table.getn(tBombersOfTechLevel)
                if bDebugMessages == true then LOG(sFunctionRef..': iAvailableBombers='..iAvailableBombers..'; strategy='..aiBrain[M27Overseer.refiAIBrainCurrentStrategy]) end
                ...
                elseif aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyACUKill and M27UnitInfo.IsUnitValid(aiBrain[M27Overseer.refoLastNearestACU]) and (M27UnitInfo.IsUnitUnderwater(aiBrain[M27Overseer.refoLastNearestACU]) or (iTechLevel == 4 and not(M27MapInfo.IsUnderwater({aiBrain[M27Overseer.refoLastNearestACU]:GetPosition()[1], aiBrain[M27Overseer.refoLastNearestACU]:GetPosition()[2] + 18, aiBrain[M27Overseer.refoLastNearestACU]:GetPosition()[3]}, false)))) then
                    ...
                    --Air domination - consider all enemy ground AA
                    if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyAirDominance then
                        iCurPriority = 1
                        local iCategoryToSearch = M27UnitInfo.refCategoryGroundAA
                        if iTechLevel == 4 then iCategoryToSearch = iCategoryToSearch * categories.TECH3 end

                        tPotentialTargets = aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryGroundAA, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], aiBrain[refiMaxScoutRadius], 'Enemy')
                        for iUnit, oUnit in tPotentialTargets do
                    ...
                    if iTechLevel == 1 or (iTechLevel == 2 and not (tbConsideredTargetsByTech[1])) then
                        ...
                        --All structures within emergency defence range - 20
                        iCurPriority = 4
                        ...
                        iCurPriority = 5
                        if iTechLevel == 1 and iTargetCount == 0 and iAvailableBombers >= 3 and aiBrain[M27Overseer.refiOurHighestAirFactoryTech] < 3 and GetGameTimeSeconds() - (aiBrain[refiTimeOfLastT1BomberMexAttack] or 0) >= 300 then
                            if bDebugMessages == true then LOG(sFunctionRef..': It has been a while since the last mex attack so will consider targeting a mex.  aiBrain[M27Overseer.refiOurHighestAirFactoryTech]='..aiBrain[M27Overseer.refiOurHighestAirFactoryTech]..'; Time since last targeted mex='..GetGameTimeSeconds() - (aiBrain[refiTimeOfLastT1BomberMexAttack] or 0)) end
                            local iClosestDistance = 100000
                            local oClosestDistance
                            ...
                            if M27Utilities.IsTableEmpty(tPotentialTargets) == false then
                                tEnemyAA = aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryGroundAA, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], aiBrain[refiMaxScoutRadius], 'Enemy')
                                ...
                                for iUnit, oUnit in tPotentialTargets do
                                    if not(M27UnitInfo.IsUnitUnderwater(oUnit)) then
                                        iCurModDistance = M27Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), tBasePosition) - M27Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain))
                                        if iCurModDistance < iClosestDistance and (not(bAvoidCruisers) or iCurModDistance < iNearestCruiserModDistance - 30) then
                                            if not(M27Logic.IsTargetUnderShield(aiBrain, oUnit, 0, false, false, true)) then
                                                if not(IsTargetCoveredByAA(oUnit, tEnemyAA, 1, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], false)) then
                                                    oClosestDistance = oUnit
                    ...
                    if iTechLevel == 2 then
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': tbConsideredTargetsByTech[1]='..tostring(tbConsideredTargetsByTech[1])..'; iTargetCount='..iTargetCount) end
                        if iTargetCount == 0 then
                            ...
                            if bDebugMessages == true then LOG(sFunctionRef..': Target count is 0. bHaveT2EarlyBombers='..tostring(bHaveT2EarlyBombers)) end
                            if bHaveT2EarlyBombers then
                                ...
                                if M27Utilities.IsTableEmpty(tEnemyT2Mex) == false then
                                    if M27Utilities.IsTableEmpty(tEnemyAA) then tEnemyAA = aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryGroundAA, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], aiBrain[refiMaxScoutRadius], 'Enemy') end
                                    tStartPoint = M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber]
                                    for iMex, oMex in tEnemyT2Mex do
                                        if bDebugMessages == true then LOG(sFunctionRef..': Early t2 bomber logic - Considering mex '..oMex.UnitId..M27UnitInfo.GetUnitLifetimeCount(oMex)..'; is it underwater='..tostring(M27UnitInfo.IsUnitUnderwater(oMex))..'; Is under shield='..tostring(M27Logic.IsTargetUnderShield(aiBrain, oMex, 0, false, false, true))..'; is protected by AA='..tostring(IsTargetCoveredByAA(oMex, tEnemyAA, 3, tStartPoint))) end
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': Have early T2 bombers so added T2 mexes without AA coverage to the target list. iTargetCount='..iTargetCount) end
                            end
                        end
                    elseif iTechLevel == 3 then
                            ...
                            for iValue, tValue in aiBrain[reftBomberShortlistByTech][1] do
                                if tValue[refiShortlistPriority] <= 3 then
                                    ...
                                    if not(EntityCategoryContains(categories.TECH1 + M27UnitInfo.refCategoryEngineer - categories.COMMAND, tValue[refiShortlistUnit].UnitId)) then
                                        ...
                                        elseif iCurPriority == 2 then iCurPriority = 3
                                        end
                                        AddUnitToShortlist(tValue[refiShortlistUnit], iTechLevel, tValue[refiBomberDefenceModDistance], tValue[refiShortlistStrikeDamageWanted])
                                    end
                                end
                        ...
                        elseif tbConsideredTargetsByTech[2] then
                            ...
                            --Check for nearby enemy T2+T3 units
                            iCurPriority = 1
                            tPotentialTargets = aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryGroundAA - categories.TECH1 - M27UnitInfo.refCategoryEngineer, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], iMaxPossibleRange, 'Enemy')
                            for iUnit, oUnit in tPotentialTargets do
                                if not (oUnit:IsUnitState('Attached')) then
                                    iCurModDistance = M27Overseer.GetDistanceFromStartAdjustedForDistanceFromMid(aiBrain, oUnit:GetPosition(), false) + math.min(100, (oUnit[refiFailedHitCount] or 0) * 15)
                                    if bDebugMessages == true then LOG(sFunctionRef..': CurPriority='..iCurPriority..'; Considering whether to add unit '..oUnit.UnitId..M27UnitInfo.GetUnitLifetimeCount(oUnit)..' to shortlist; mod distance='..(iCurModDistance or 'nil')..'; aiBrain[refiBomberDefenceModDistance]='..aiBrain[refiBomberDefenceModDistance]..'; Actual distance to start='..M27Utilities.GetDistanceBetweenPositions(M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], oUnit:GetPosition())..'; Angle from start to unit='..M27Utilities.GetAngleFromAToB(M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], oUnit:GetPosition())..'; Dist to enemy base='..aiBrain[M27Overseer.refiDistanceToNearestEnemyBase]..'; Angle to enemy base from start='..M27Utilities.GetAngleFromAToB(M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain))..'; Unit position='..repr(oUnit:GetPosition())..'; enemy base position='..repr(M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain))) end
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': iTargetCount after considering emergency threat range='..iTargetCount..'; is table of enemy T2 mexes empty='..tostring(M27Utilities.IsTableEmpty(tEnemyT2Mex))..'; is table of enemy t3 air empty='..tostring(M27Utilities.IsTableEmpty(tEnemyAA))..'; iHighestPriorityFound='..iHighestPriorityFound) end
                        if M27Utilities.IsTableEmpty(tEnemyT2Mex) == false then
                            iCurPriority = iHighestPriorityFound --I.e. consider alongside the top priority for defence

                            for iMex, oMex in tEnemyT2Mex do
                                if bDebugMessages == true then LOG(sFunctionRef..': Considering mex '..oMex.UnitId..M27UnitInfo.GetUnitLifetimeCount(oMex)..'; is it underwater='..tostring(M27UnitInfo.IsUnitUnderwater(oMex))..'; Is under shield='..tostring(M27Logic.IsTargetUnderShield(aiBrain, oMex, 0, false, false, true))..'; is protected by AA='..tostring(IsTargetCoveredByAA(oMex, tEnemyAA, 3, tStartPoint))) end
                                if not(M27UnitInfo.IsUnitUnderwater(oMex)) then
                                    --Include if its unshielded with no AA on the way to it from our base (assume part-compelte shields will be built by the time we get there)
                                    if not(M27Logic.IsTargetUnderShield(aiBrain, oMex, 0, false, false, true)) then
                                        if not(IsTargetCoveredByAA(oMex, tEnemyAA, 3, tStartPoint)) then
                                            if bDebugMessages == true then LOG(sFunctionRef..': Will add mex '..oMex.UnitId..M27UnitInfo.GetUnitLifetimeCount(oMex)..' to shortlist') end
                                            AddUnitToShortlist(oMex, iTechLevel)
                                        end
                                    end
                                end
                        ...
                        --If enemy cruiser within emergency defence range then also consider this (but not carriers)
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': If no target count then will search for lower priority targets. iTargetCount='..iTargetCount) end
                        if iTargetCount == 0 then
                            ...
                            if bDebugMessages == true then LOG(sFunctionRef..': is tEnemiesEmpty='..tostring(M27Utilities.IsTableEmpty(tEnemies))) end
                            if M27Utilities.IsTableEmpty(tEnemies) == false then
                                for iUnit, oUnit in tEnemies do
                                    if bDebugMessages == true then LOG(sFunctionRef..': Considering oUnit '..oUnit.UnitId..M27UnitInfo.GetUnitLifetimeCount(oUnit)..'; is it underwater='..tostring(M27UnitInfo.IsUnitUnderwater(oUnit))..'; Is under shield='..tostring(M27Logic.IsTargetUnderShield(aiBrain, oUnit, 0, false, false, true))..'; is protected by AA='..tostring(IsTargetCoveredByAA(oUnit, tEnemyAA, 3, tStartPoint))) end
                                    if not(M27UnitInfo.IsUnitUnderwater(oUnit)) then
                                        --Include if its unshielded with no AA on the way to it from our base (assume part-compelte shields will be built by the time we get there)
                                        if not(M27Logic.IsTargetUnderShield(aiBrain, oUnit, 0, false, false, true)) then
                                            if not(IsTargetCoveredByAA(oUnit, tEnemyAA, 3, tStartPoint)) then
                                                AddUnitToShortlist(oUnit, iTechLevel)
                                                if bDebugMessages == true then LOG(sFunctionRef..': Just tried adding the unit to shortlist') end
                                            end
                                        end
                                    end
                                end
                            end
                            if bDebugMessages == true then LOG(sFunctionRef..': iTargetCount='..iTargetCount..'; if its zero will look for enemy ACUs and experimentals closer to our base than enemy base') end
                            if iTargetCount == 0 then
                                iCurPriority = 5
                                if bDebugMessages == true then LOG(sFunctionRef..': Couldnt find any emergengy defence or t2 vulnerable mexes so have looked for enemy ACU and land experimentals on our side of the map.  iTargetCount='..iTargetCount) end
                                ...
                                if M27Utilities.IsTableEmpty(tEnemyAA) == false then tEnemyNonMAASAM = aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryGroundAA * categories.TECH3 * categories.STRUCTURE + M27UnitInfo.refCategoryCruiserCarrier, tStartPoint, aiBrain[refiMaxScoutRadius], 'Enemy') end
                                if M27Utilities.IsTableEmpty(tEnemies) == false then
                                    for iUnit, oUnit in tEnemies do
                                        if (oUnit[refiStrikeDamageAssigned] or 0) == 0 then --Dont want to target if already being targeted for some other reason (as risk sending bombers piecemeal)
                                            if not(M27UnitInfo.IsUnitUnderwater(oUnit)) then
                                                if not(M27Logic.IsTargetUnderShield(aiBrain, oUnit, 0, false, false, true)) and not(IsTargetCoveredByAA(oUnit, tEnemyNonMAASAM, 3, tStartPoint)) then
                                                    AddUnitToShortlist(oUnit, iTechLevel)
                                                end
                                            end
                                        end
                            ...
                            if iTargetCount == 0 then
                                ...
                                if M27Utilities.IsTableEmpty(tEnemies) == false then
                                    for iUnit, oUnit in tEnemies do
                                        if bDebugMessages == true then LOG(sFunctionRef..': Considering oUnit '..oUnit.UnitId..M27UnitInfo.GetUnitLifetimeCount(oUnit)..'; is it underwater='..tostring(M27UnitInfo.IsUnitUnderwater(oUnit))..'; Is under shield='..tostring(M27Logic.IsTargetUnderShield(aiBrain, oUnit, 0, false, false, true))..'; is protected by AA='..tostring(IsTargetCoveredByAA(oUnit, tEnemyAA, 3, tStartPoint))..'; strike damage already assigned='..(oUnit[refiStrikeDamageAssigned] or 0)) end
                                        if not(M27UnitInfo.IsUnitUnderwater(oUnit)) then
                                    ...
                                    if M27Utilities.IsTableEmpty(tEnemies) == false then
                                        for iUnit, oUnit in tEnemies do
                                            if (oUnit[refiStrikeDamageAssigned] or 0) == 0 then
                                                ...
                                                if not (M27UnitInfo.IsUnitUnderwater(oUnit)) then
                                                    if not(M27Logic.IsTargetUnderShield(aiBrain, oUnit, 0, false, false, true)) and not (IsTargetCoveredByAA(oUnit, tEnemyAA, 3, tStartPoint)) then
                                                        AddUnitToShortlist(oUnit, iTechLevel)
                                                    end
                                                end
                                            end
                        ...
                        elseif bDebugMessages == true then LOG(sFunctionRef..': Have targets from emergency defence and t2 vulnerable mexes so wont look for more targets. iTargetCount='..iTargetCount..'; tStartPoint='..repr(tStartPoint))
                        end
                    elseif iTechLevel == 4 then
                        ...
                        tEnemyAA = aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryGroundAA * categories.TECH3 + M27UnitInfo.refCategoryCruiser, tStartPoint, aiBrain[refiMaxScoutRadius], 'Enemy')
                        if bDebugMessages == true then
                            if not(M27Utilities.IsTableEmpty(tEnemyAA)) then
                                LOG(sFunctionRef..': Size of tEnemyAA='..table.getn(tEnemyAA))
                            else LOG(sFunctionRef..': tENemyAA is empty')
                            end
                        end
                        ...
                        if ScenarioInfo.Options.Victory == "demoralization" and M27UnitInfo.IsUnitValid(aiBrain[M27Overseer.refoLastNearestACU]) and M27Utilities.GetDistanceBetweenPositions(aiBrain[M27Overseer.reftLastNearestACU], aiBrain[M27Overseer.refoLastNearestACU]:GetPosition()) <= 10 and (not(M27UnitInfo.IsUnitUnderwater(aiBrain[M27Overseer.refoLastNearestACU])) or not(M27MapInfo.IsUnderwater({aiBrain[M27Overseer.refoLastNearestACU]:GetPosition()[1], aiBrain[M27Overseer.refoLastNearestACU]:GetPosition()[2] + 18, aiBrain[M27Overseer.refoLastNearestACU]:GetPosition()[3]}, false))) then
                            ...
                            if M27Utilities.IsTableEmpty(tNearbyFriendlies) then
                                iEnemyAANearTarget = IsTargetCoveredByAA(aiBrain[M27Overseer.refoLastNearestACU], tEnemyAA, 3, tStartPoint, true)
                                if bDebugMessages == true then LOG(sFunctionRef..': Checking ACU nearby, iEnemyAANearTarget='..iEnemyAANearTarget) end
                        ...
                        if iTargetCount == 0 then
                            ...
                            if bDebugMessages == true then LOG(sFunctionRef..': Checking high value structure threats; is table empty='..tostring(M27Utilities.IsTableEmpty(tEnemies))) end
                            if M27Utilities.IsTableEmpty(tEnemies) == false then
                                for iUnit, oUnit in tEnemies do
                                    if oUnit:GetFractionComplete() >= 0.5 then
                                        ...
                                        if M27Utilities.IsTableEmpty(tNearbyFriendlies) then
                                            iEnemyAANearTarget = IsTargetCoveredByAA(oUnit, tEnemyAA, 3, tStartPoint, true)
                            ...
                            if iTargetCount == 0 then
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': Checking enemy naval threats; is table empty='..tostring(M27Utilities.IsTableEmpty(tEnemies))) end
                                if M27Utilities.IsTableEmpty(tEnemies) == false then
                                    for iUnit, oUnit in tEnemies do
                                        if oUnit:GetFractionComplete() >= 0.5 then
                                            tNearbyFriendlies = aiBrain:GetUnitsAroundPoint(iNearbyFriendlyCategories, oUnit:GetPosition(), iFriendlySearchRange, 'Ally')
                            ...
                            if iTargetCount == 0 then
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': Checking enemy vulnerable mexes; is table empty='..tostring(M27Utilities.IsTableEmpty(tEnemies))) end
                                local tNearbyMexes
                                if M27Utilities.IsTableEmpty(tEnemies) == false then
                                    for iUnit, oUnit in tEnemies do
                                        if bDebugMessages == true then LOG(sFunctionRef..': Considering oUnit='..oUnit.UnitId..M27UnitInfo.GetUnitLifetimeCount(oUnit)..'; Fractioncomplete='..oUnit:GetFractionComplete()..'; will check for how many high value targets are within iAOE*1.2='..iAOE*1.2..' of the position '..repr(oUnit:GetPosition())) end
                                        if oUnit:GetFractionComplete() >= 0.5 then
                                            tNearbyFriendlies = aiBrain:GetUnitsAroundPoint(iNearbyFriendlyCategories, oUnit:GetPosition(), iFriendlySearchRange, 'Ally')
                                            if M27Utilities.IsTableEmpty(tNearbyFriendlies) then
                                                ...
                                                if bDebugMessages == true then LOG(sFunctionRef..': size of tNearbyMexes='..table.getn(tNearbyMexes)) end
                                                if table.getn(tNearbyMexes) >= 3 then
                                                    iEnemyAANearTarget = IsTargetCoveredByAA(oUnit, tEnemySAMs, 3, tStartPoint, true)
                                                    if bDebugMessages == true then LOG(sFunctionRef..': iEnemyAANearTarget='..iEnemyAANearTarget) end
                            ...
                            if iTargetCount == 0 then
                                    ...
                                    for iUnit, oUnit in tEnemies do
                                        ...
                                        if oUnit:GetFractionComplete() >= 0.5 then
                                            ...
                                            if M27Utilities.IsTableEmpty(tNearbyFriendlies) then
                                                ...
                                                if bDebugMessages == true then LOG(sFunctionRef..': size of tNearbyMexes='..table.getn(tNearbyOtherUnits)) end
                                ...
                                if iTargetCount == 0 then
                                    if bDebugMessages == true then LOG(sFunctionRef..': Dont ahve any targets so will go for the nearest SAM to us') end
                                    if not(M27Utilities.IsTableEmpty(tEnemyAA)) then
                                        iCurPriority = 5
                    ...
                    if iAvailableBombers >= 10 and iTechLevel == 1 then
                        ...
                        for iEntry, tValue in M27Utilities.SortTableBySubtable(aiBrain[reftBomberShortlistByTech][iTechLevel], refiBomberDefenceModDistance, true) do
                            ...
                            while (oCurTarget[refiStrikeDamageAssigned] <= tValue[refiShortlistStrikeDamageWanted] or EntityCategoryContains(categories.COMMAND + categories.EXPERIMENTAL, tValue[refiShortlistUnit].UnitId)) do
                                if bDebugMessages == true then LOG(sFunctionRef..': oCurTarget='..oCurTarget.UnitId..M27UnitInfo.GetUnitLifetimeCount(oCurTarget)..'; strike damage assigned='..(oCurTarget[refiStrikeDamageAssigned] or 'nil')..'; tValue[refiShortlistStrikeDamageWanted]='..tValue[refiShortlistStrikeDamageWanted]..'; iAvailableBombers='..iAvailableBombers..'; iLoopStartAvailableBombers='..iLoopStartAvailableBombers) end
                                iLoopStartAvailableBombers = iAvailableBombers
                                oCurBomber = tBombersOfTechLevel[iAvailableBombers]
                                if not(M27UnitInfo.IsUnitValid(oCurBomber)) then M27Utilities.ErrorHandler('Returned a bomber iAvailablerBombers='..iAvailableBombers..' that isnt valid') end
                                TargetUnit(oCurTarget, oCurBomber, tValue[refiShortlistPriority], tValue[refiShortlistStrikeDamageWanted])
                                if EntityCategoryContains(M27UnitInfo.refCategoryT1Mex, oCurTarget.UnitId) then
                            ...
                            if iAvailableBombers <= 0 then break end
                        end
                    else
                        ...
                        while iAvailableBombers > 0 do
                            ...
                            if bDebugMessages == true then LOG(sFunctionRef..': About to assign target to oCurBomber='..oCurBomber.UnitId..M27UnitInfo.GetUnitLifetimeCount(oCurBomber)) end
                            for iEntry, tSubtable in aiBrain[reftBomberShortlistByTech][iTechLevel] do
                                if M27UnitInfo.IsUnitValid(tSubtable[refiShortlistPriority]) then --Redundancy as once had error that indicated oClosestDistance was a table
                                    if (tSubtable[refiShortlistUnit][refiStrikeDamageAssigned] or 0) <= tSubtable[refiShortlistStrikeDamageWanted] then
                                        if tSubtable[refiShortlistPriority] <= iLowestPriority then
                                            ...
                                            if bDebugMessages == true then LOG(sFunctionRef..': Current entry in shortlist is low enough priority.  tSubtable[refiShortlistPriority]='..tSubtable[refiShortlistPriority]..'; iCurModDistance='..iCurModDistance..'; iLowestPriority='..iLowestPriority..'; iClosestDistance='..iClosestDistance..'; tSubtable[refiShortlistUnit][refiStrikeDamageAssigned]='..(tSubtable[refiShortlistUnit][refiStrikeDamageAssigned] or 'nil')) end
                                            if tSubtable[refiShortlistPriority] < iLowestPriority then
                                                iClosestDistance = iCurModDistance
                                                oClosestDistance = tSubtable[refiShortlistUnit]
                                                iTargetMaxHealth = tSubtable[refiShortlistStrikeDamageWanted]
                                                if bDebugMessages == true then LOG(sFunctionRef..': New target selected='..oClosestDistance.UnitId..M27UnitInfo.GetUnitLifetimeCount(oClosestDistance)) end
                            ...
                            if oClosestDistance then
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': Telling oCurBomber='..oCurBomber.UnitId..M27UnitInfo.GetUnitLifetimeCount(oCurBomber)..' to target unit '..oClosestDistance.UnitId..M27UnitInfo.GetUnitLifetimeCount(oClosestDistance)..' with strike damage assigned on that unit='..oClosestDistance[refiStrikeDamageAssigned]..' and max strike damage wanted='..oClosestDistance[refiMaxStrikeDamageWanted]..'; bomber on assignment='..tostring(oCurBomber[refbOnAssignment])) end
                            end

                            if iLoopStartAvailableBombers <= iAvailableBombers or iAvailableBombers == 0 then
                                --We havent found any new targets so abort
                                if bDebugMessages == true then LOG(sFunctionRef..': iLoopStartAvailableBombers='..iLoopStartAvailableBombers..'; iAvailableBombers='..iAvailableBombers..'; will abort') end
                                break
                            end
                        end
                    end
            ...
            if iAvailableBombers > 0 then
                if bDebugMessages == true then LOG(sFunctionRef..': Available bombers after assigning all targets='..iAvailableBombers..'; will send back to rally point') end
                ...
                if iTechLevel == 3 and iAvailableBombers >= 3 and iTargetCount == 0 then
                    ...
                    if M27Utilities.IsTableEmpty(tStartPoint) then tStartPoint = M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber] end
                    ...
                    if iStratsNearRallyPoint >= 3 then
                        ...
                        if ScenarioInfo.Options.Victory == "demoralization" then bAssassination = true end
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': tStartPoint='..repr((tStartPoint or {'nil'}))..'; aiBrain[M27Overseer.refiDistanceToNearestEnemyBase]='..(aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] or 'nil')..'; aiBrain[refiMaxScoutRadius]='..(aiBrain[refiMaxScoutRadius] or 'nil')..'; tEnemies is high values tructures (e.g. t2 defence and t3 structures, excl t3 aa) and enemy ACU.  Is the table empty='..tostring(M27Utilities.IsTableEmpty(tEnemies))) end
                        if M27Utilities.IsTableEmpty(tEnemies) == false then
                            if M27Utilities.IsTableEmpty(tEnemyAA) then
                                tEnemyAA = aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryGroundAA * categories.TECH3 + M27UnitInfo.refCategoryCruiser, tStartPoint, aiBrain[refiMaxScoutRadius], 'Enemy')
                            end

                            for iUnit, oUnit in tEnemies do
                                if bDebugMessages == true then LOG(sFunctionRef..': Considering oUnit='..oUnit.UnitId..M27UnitInfo.GetUnitLifetimeCount(oUnit)..'; Strike damage already assigned='..(oUnit[refiStrikeDamageAssigned] or 'nil')..'; Is unit underwater='..tostring(M27UnitInfo.IsUnitUnderwater(oUnit))..'; AA covering target='..IsTargetCoveredByAA(oUnit, tEnemyAA, 3, tStartPoint, true)..'; iStratsNearRallyPoint='..iStratsNearRallyPoint) end
                                if (oUnit[refiStrikeDamageAssigned] or 0) <= 1000 then --Dont want to target if already being targeted for some other reason (as risk sending bombers piecemeal); since dealing with t3 strats can ignore low level bomber targeting though
                                    if not(M27UnitInfo.IsUnitUnderwater(oUnit)) then
                                        if not(IsTargetCoveredByAA(oUnit, tEnemyAA, 3, tStartPoint)) or (bAssassination and EntityCategoryContains(categories.COMMAND, oUnit.UnitId) and (iStratsNearRallyPoint >= 26 and IsTargetCoveredByAA(oUnit, tEnemyAA, 3, tStartPoint, true) <= 8) or (iStratsNearRallyPoint < 26 and iStratsNearRallyPoint >= 6 and IsTargetCoveredByAA(oUnit, tEnemyAA, 3, tStartPoint, true) <= 4)) or (iStratsNearRallyPoint >= 15 and EntityCategoryContains(categories.EXPERIMENTAL, oUnit.UnitId) and IsTargetCoveredByAA(oUnit, tEnemyAA, 3, tStartPoint, true) <= 2) then
                                            iCurStrikeDamage = GetMaxStrikeDamageWanted(oUnit)
                        ...
                        if iStratsNearRallyPoint >= 9 and (iPossibleTargets == 0 or (M27Utilities.IsTableEmpty(tTargetsByMaxHealth[1]) and M27Utilities.IsTableEmpty(tTargetsByMaxHealth[2]))) then
                            --Consider targeting unshielded SAMs and cruisers if we have lots of available strats
                            ...
                            if bDebugMessages == true then LOG(sFunctionRef..': About to consider tactics for if have loads of idle strats. iPossibleTargets='..iPossibleTargets..'; iStratsNearRallyPoint='..iStratsNearRallyPoint) end
                            if iPossibleTargets == 0 and iStratsNearRallyPoint >= 30 then
                                ...
                                if ScenarioInfo.Options.Victory == "demoralization" then
                                    ...
                                    if bDebugMessages == true then LOG(sFunctionRef..': Seeing if enemy ACU can be identified. tEnemies is empty='..tostring(M27Utilities.IsTableEmpty(tEnemies))) end
                                    if M27Utilities.IsTableEmpty(tEnemies) == false then
                                        for iUnit, oUnit in tEnemies do
                                            if bDebugMessages == true then LOG(sFunctionRef..': Considering ACU unit '..oUnit.UnitId..M27UnitInfo.GetUnitLifetimeCount(oUnit)..'; Is underwater='..tostring(M27UnitInfo.IsUnitUnderwater(oUnit))..'; Strike damage that would want='..GetMaxStrikeDamageWanted(oUnit)) end
                                            if not (M27UnitInfo.IsUnitUnderwater(oUnit)) then
                                                iCurStrikeDamage = GetMaxStrikeDamageWanted(oUnit)
                                                iTableIndex = math.min(5, math.ceil(iCurStrikeDamage / 10000))
                                                table.insert(tTargetsByMaxHealth[iTableIndex], oUnit)
                                                table.insert(tTargetStrikeDamageByMaxHealth[iTableIndex], iCurStrikeDamage)
                                                iPossibleTargets = iPossibleTargets + 1
                                                if bDebugMessages == true then LOG(sFunctionRef..': Adding ACU to list of targets. iPossibleTargets='..iPossibleTargets) end
                                            end
                                        end
                                    end
                                end
                                ...
                                if iPossibleTargets == 0 and M27Utilities.IsTableEmpty(tEnemyAA) == false then
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': iStratsNearRallyPoint='..iStratsNearRallyPoint..'; coordinated targets to consider='..iPossibleTargets..'; will decide on best target if have any') end
                        if iPossibleTargets > 0 then
                            --Chose a target for a coordinated attack; start with lowest priority
                            for iHealthIndex = 1, 5 do
                                if bDebugMessages == true then LOG(sFunctionRef..': Considering all units for iHealthIndex='..iHealthIndex..'; iTargetCount='..iTargetCount..'; Is table for this healthindex empty='..tostring(M27Utilities.IsTableEmpty(tTargetsByMaxHealth[iHealthIndex]))) end
                                if M27Utilities.IsTableEmpty(tTargetsByMaxHealth[iHealthIndex]) == false then
                                    if bDebugMessages == true then LOG(sFunctionRef..': iTechLevel='..(iTechLevel or 'nil')..'; iHealthIndex='..iHealthIndex..'; Size of table of targets by max health='..table.getn(tTargetsByMaxHealth[iHealthIndex])) end
                                ...
                                if iTargetCount > 0 then break end
                            end
                            if bDebugMessages == true then LOG(sFunctionRef..': iPossibleTarget='..iPossibleTargets..'; iTargetCount='..iTargetCount..'; iStratsNearRallyPoint='..iStratsNearRallyPoint) end
                            ...
                            local iStrikeDamageWanted
                            if bDebugMessages == true then LOG(sFunctionRef..': About to send strats on coordinated attack. iStratsNearRallyPoint='..iStratsNearRallyPoint..'; iTargetCount='..iTargetCount) end
                            ...
                            if iStratsNearRallyPoint > 0 then iAverageStrikeDamage = iTotalStrikeDamage / iStratsNearRallyPoint end
                            ...
                            while iStratsNearRallyPoint > 0 do
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': iStratsNearRallyPoint='..iStratsNearRallyPoint..'; iClosestDistance='..iClosestDistance) end
                                ...
                                if oClosestDistance then
                                    ...
                                    if bDebugMessages == true then LOG(sFunctionRef..': Issuing coordinated attack on oClosestDistance='..oClosestDistance.UnitId..M27UnitInfo.GetUnitLifetimeCount(oClosestDistance)..' if we have enough strike damage; iStrikeDamageWanted='..iStrikeDamageWanted..'; iRemainingStrikeDamageAvailable='..iRemainingStrikeDamageAvailable) end
                                    if iRemainingStrikeDamageAvailable >= math.min(75000, iStrikeDamageWanted) or oClosestDistance[refiStrikeDamageAssigned] > 1000 then
                                        ...
                                        while oClosestDistance[refiStrikeDamageAssigned] <= iStrikeDamageWanted do
                                            ...
                                            iStratsNearRallyPoint = iStratsNearRallyPoint - 1
                                            if bDebugMessages == true then LOG(sFunctionRef..': Told bomber '..oCurBomber.UnitId..M27UnitInfo.GetUnitLifetimeCount(oCurBomber)..' to target unit '..oClosestDistance.UnitId..M27UnitInfo.GetUnitLifetimeCount(oClosestDistance)..'; iStratsNearRallyPoint after this='..iStratsNearRallyPoint) end
                                            if iStratsNearRallyPoint <= 0 then break end
                                        end
                                    else
                                ...
                                if iLoopStartAvailableBombers <= iStratsNearRallyPoint or iStratsNearRallyPoint == 0 then
                                    --We havent found any new targets so abort
                                    if bDebugMessages == true then LOG(sFunctionRef..': No new targets so will abort loop now') end
                                    break
                                end
                            end
                        end
                ...
                if iAvailableBombers > 0 then
                    ...
                    for iBomber, oBomber in tBombersOfTechLevel do
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': oBomber='..oBomber.UnitId..M27UnitInfo.GetUnitLifetimeCount(oBomber)..'; tCurTarget='..repr((tCurTarget or {'nil'}))..'; tRallyPoint='..repr((tRallyPoint or {'nil'}))..'; oBomber position='..repr(oBomber:GetPosition())) end
                        if (M27Utilities.IsTableEmpty(tCurTarget) == false and (tCurTarget[1] == tRallyPoint[1] and tCurTarget[3] == tRallyPoint[3])) or M27Utilities.GetDistanceBetweenPositions(oBomber:GetPosition(), tRallyPoint) <= 30 then
                            ...
                            if bDebugMessages == true then
                                LOG(sFunctionRef .. ': Bomber already near or moving to rally point; distance to rally point=' .. repr(tRallyPoint) .. '; tCurTarget=' .. repr(tCurTarget))
                            end
                        else
                            --Move to nearest rally point
                            if bDebugMessages == true then
                                LOG(sFunctionRef .. ': Telling ' .. oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber) .. ' to move to nearest rapply point=' .. repr(tRallyPoint))
                            end
                            IssueClearCommands({ oBomber })
...
function AirBomberManagerOld(aiBrain)
    local bDebugMessages = false if M27Utilities.bGlobalDebugOverride == true then   bDebugMessages = true end
    local sFunctionRef = 'AirBomberManager'
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerStart)
    ...
    if aiBrain[M27Overseer.refiOurHighestAirFactoryTech] >= 3 then
        iMaxTargetsForBomber = 1
    end

    if aiBrain[refbShortlistContainsLowPriorityTargets] == true or aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyAirDominance then
        iMaxTargetsForBomber = 1
    end --Dont want to issue low priority targets unless bomber is idle
    if aiBrain[refbShortlistContainsLowPriorityTargets] then
        iMaxTargetsForBomber = 1
    end
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': Start of code; IsAvailableTorpBombersEmpty=' .. tostring(M27Utilities.IsTableEmpty(aiBrain[reftAvailableTorpBombers])))
    end
    local bTargetGround
    local tGroundTarget
    --if M27Utilities.IsTableEmpty(aiBrain[reftAvailableBombers]) == false then
    ...
    if M27Utilities.IsTableEmpty(aiBrain[reftBomberTargetShortlist]) == false and M27Utilities.IsTableEmpty(aiBrain[reftAvailableBombers]) == false then
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': Have a target shortlist and available bombers, so will now cycle through targets and assign bombers to them')
        end
        ...
        if oBomber[refiCurTargetNumber] == nil or oBomber[refiCurTargetNumber] == 0 then
            ...
            if M27Config.M27ShowUnitNames == true and oBomber.GetUnitId then M27PlatoonUtilities.UpdateUnitNames({oBomber}, oBomber.UnitId..M27UnitInfo.GetUnitLifetimeCount(oBomber)..':NoTargetReturnToBase') end
        end
    else --]]
            ...
            if iUnassignedTargets > 0 then
                if oBomber[refiCurTargetNumber] == nil or oBomber[refiCurTargetNumber] == 0 then
                    ...
                    oBomber[reftTargetList] = {}
                    if M27Config.M27ShowUnitNames == true and oBomber.GetUnitId then
                        M27PlatoonUtilities.UpdateUnitNames({ oBomber }, oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber) .. ':Cleared')
                    end
                    if bDebugMessages == true then
                        LOG(sFunctionRef .. ': Clearing bomber commands as it has no target number')
                    end
                else
                    if (oBomber[reftTargetList][oBomber[refiCurTargetNumber]][refiShortlistUnit][refiStrikeDamageAssigned] or 0) < GetMaxStrikeDamageWanted(oBomber[reftTargetList][oBomber[refiCurTargetNumber]][refiShortlistUnit]) then
                        ...
                        if bDebugMessages == true then
                            LOG(sFunctionRef .. ': Bomber wont deal enough strike damage to deal with curernt target so no point looking for new targets yet')
                        end
                    else
                ...
                iCurLoopCount = 0
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': iBomberRemainingTargetCount=' .. iBomberRemainingTargetCount .. '; iMaxTargetsForBomber=' .. iMaxTargetsForBomber .. '; iUnassignedTargets=' .. iUnassignedTargets)
                end
                while iBomberRemainingTargetCount < iMaxTargetsForBomber do
                    ...
                    if iUnassignedTargets > 0 then
                        ...
                        if bDebugMessages == true then
                            LOG(sFunctionRef .. ': iUnassignedTargets=' .. iUnassignedTargets .. ' so will get the closest one to the bomber for it to target')
                        end
                        ...
                        if M27Utilities.IsTableEmpty(aiBrain[reftBomberTargetShortlist]) == false then
                            ...
                            if oClosestNewTarget then
                                if bDebugMessages == true then
                                    LOG(sFunctionRef .. ': About to Update bomber target to oClosestNewTarget=' .. oClosestNewTarget.UnitId .. '; aiBrain[refiShieldIgnoreValue]=' .. aiBrain[refiShieldIgnoreValue] .. '; cur target priority=' .. aiBrain[reftBomberTargetShortlist][iClosestTargetShortlistKey][refiShortlistPriority] .. '; low priority threshold=' .. aiBrain[refiLowPriorityStart])
                                end
                                ...
                                if bDebugMessages == true then
                                    LOG(sFunctionRef .. ': About to consider removal of the target from the shortlist')
                                end
                                if ConsiderRemovalFromShortlist(aiBrain, iClosestTargetShortlistKey) then
                                    iUnassignedTargets = iUnassignedTargets - 1
                                end
                                --Does our bomber have good aoe and is targetting a structure?
                                TellBomberToAttackTarget(oBomber, oClosestNewTarget, true)
                                ...
                                if aiBrain[reftBomberTargetShortlist][iClosestTargetShortlistKey][refiShortlistPriority] >= aiBrain[refiLowPriorityStart] then
                                    ...
                                    aiBrain[reftBombersAssignedLowPriorityTargets][oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber)] = oBomber
                                    if bDebugMessages == true then
                                        LOG(sFunctionRef .. ': Added ' .. aiBrain[reftBombersAssignedLowPriorityTargets][oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber)].UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber) .. ' to the table of bombers with low priority targets')
                                    end
                                else
                                    --Have a high priority, check if the bomber is in the table of low priority targets, and if iti s then remove it
                                    if M27Utilities.IsTableEmpty(aiBrain[reftBombersAssignedLowPriorityTargets]) == false then
                                        if bDebugMessages == true then
                                            LOG(sFunctionRef .. ': Removing ' .. oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber) .. ' from the table of bombers with low priority targets')
                                        end
                                        aiBrain[reftBombersAssignedLowPriorityTargets][oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber)] = nil
                                    end
                                end
                    ...
                    elseif iBomberRemainingTargetCount == 0 then
                        iSpareBombers = math.max(1, iSpareBombers + 1)
                    end
                end
            else
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': No unassigned targets; if bomber has no current target then will increase spare bombers')
                end
        ...
        if M27Utilities.IsTableEmpty(aiBrain[reftBomberTargetShortlist]) == false then
            ...
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': Is AvailableBombersEmpty=' .. tostring(M27Utilities.IsTableEmpty(aiBrain[reftAvailableBombers])))
            end
        else
            iSpareBombers = table.getn(aiBrain[reftAvailableBombers])
        end
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': Either no target shortlist or no available bombers; iSpareBombers=' .. iSpareBombers)
        end
    end
    ...
    --Order any spare bombers that dont have a current target to go to nearest rally point; if are in air domination mode then instead send them on attack-move to enemy base
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': iSpareBombers=' .. iSpareBombers .. '; if have available bombers then will get any with no target to go to the nearest rally point')
    end
    if M27Utilities.IsTableEmpty(aiBrain[reftAvailableBombers]) == false and iSpareBombers > 0 then
        ...
        for iBomber, oBomber in aiBrain[reftAvailableBombers] do
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': Checking if bomber ' .. M27UnitInfo.GetUnitLifetimeCount(oBomber) .. ' has no target and needs to return to a rally point')
            end
            if (oBomber[refiCurTargetNumber] or 0) == 0 then

                --Add to table of bombers with shortlist target if not already on there
                if M27Utilities.IsTableEmpty(aiBrain[reftBombersAssignedLowPriorityTargets]) or not (aiBrain[reftBombersAssignedLowPriorityTargets][oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber)]) then
                    if bDebugMessages == true then
                        LOG(sFunctionRef .. ': Adding bomber ' .. oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber) .. ' to the table of units with low priority targets')
                    end
                    aiBrain[reftBombersAssignedLowPriorityTargets][oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber)] = oBomber
                end

                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': Bomber ' .. oBomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oBomber) .. ' has no target number, will check if its moving to or already near a rally point or enemy base')
                end
                if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyAirDominance then
                    tRallyPoint = { aiBrain[M27MapInfo.reftPrimaryEnemyBaseLocation][1], aiBrain[M27MapInfo.reftPrimaryEnemyBaseLocation][2], aiBrain[M27MapInfo.reftPrimaryEnemyBaseLocation][3] }
                else
...
if M27Utilities.IsTableEmpty(aiBrain[reftBomberTargetShortlist]) == false then
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': iSpareBombers=' .. iSpareBombers .. '; aiBrain[refiLargeBomberAttackThreshold]=' .. aiBrain[refiLargeBomberAttackThreshold])
    end
    if iSpareBombers and iSpareBombers < 0 then
        ...
        if iSpareBombers >= aiBrain[refiLargeBomberAttackThreshold] and not (aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyAirDominance) and GetGameTimeSeconds() >= 600 then
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': Have enough bombers for a large attack')
            end
            ...
            for iBomber, oBomber in aiBrain[reftAvailableBombers] do
                ...
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': iIdleBomberCount=' .. iIdleBomberCount)
                end
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': Strategy=' .. aiBrain[M27Overseer.refiAIBrainCurrentStrategy] .. '; M27Utilities.IsTableEmpty(aiBrain[reftAvailableTorpBombers)]=' .. tostring(M27Utilities.IsTableEmpty(aiBrain[reftAvailableTorpBombers])) .. '; M27UnitInfo.IsUnitUnderwater(aiBrain[M27Overseer.refoLastNearestACU])=' .. tostring(M27UnitInfo.IsUnitUnderwater(aiBrain[M27Overseer.refoLastNearestACU])))
    end
    if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyACUKill and M27Utilities.IsTableEmpty(aiBrain[reftAvailableTorpBombers]) == false and M27UnitInfo.IsUnitValid(aiBrain[M27Overseer.refoLastNearestACU]) and M27UnitInfo.IsUnitUnderwater(aiBrain[M27Overseer.refoLastNearestACU]) then
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': About to cycle through available bombers')
        end
        for iTorpBomber, oTorpBomber in aiBrain[reftAvailableTorpBombers] do
            UpdateBomberTargets(oTorpBomber)
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': Torp bomber LC=' .. M27UnitInfo.GetUnitLifetimeCount(oTorpBomber) .. '; CurTargetNumber=' .. (oTorpBomber[refiCurTargetNumber] or 'nil'))
            end
            if oTorpBomber[refiCurTargetNumber] == nil or oTorpBomber[refiCurTargetNumber] == 0 then
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': About to update bomber target to ACU')
                end
                ...
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': Told torp bomber to attack ACU')
                end
            end
        end
    end
...
function AirAAManager(aiBrain)
    ...
    if not (aiBrain.M27IsDefeated) and M27Logic.iTimeOfLastBrainAllDefeated < 10 then
        ...
        if not (aiBrain[refbMercySightedRecently]) and aiBrain[refiHighestEnemyAirThreat] >= 2000 and not ((aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyAirDominance or aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyACUKill)) then
            ...
            if aiBrain[refiOurMassInAirAA] < aiBrain[refiHighestEnemyAirThreat] * 0.35 then
        ...
        if M27Utilities.IsTableEmpty(tEnemyAirUnits) == false then
            if bDebugMessages == true then
                ...
                if M27Utilities.IsTableEmpty(tFriendlyT3Bombers) == false then
                    for iT3Bomber, oT3Bomber in tFriendlyT3Bombers do
                        LOG(sFunctionRef .. ': Bomber=' .. oT3Bomber.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oT3Bomber) .. '; Position=' .. repr(oT3Bomber:GetPosition()))
                        if M27Utilities.IsTableEmpty(tEnemyAirAA) == false then
                            ...
                            LOG(sFunctionRef .. ': Nearest enemy airAA=' .. oNearestEnemyAirAA.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oNearestEnemyAirAA) .. '; position=' .. repr(oNearestEnemyAirAA:GetPosition()) .. '; distance to bomber=' .. M27Utilities.GetDistanceBetweenPositions(oNearestEnemyAirAA:GetPosition(), oT3Bomber:GetPosition()))
                        else
            ...
            for iUnit, oUnit in tEnemyAirUnits do
                ...
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': Considering if we want to attack enemy unit ' .. oUnit.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oUnit) .. ' that is at position ' .. repr(oUnit:GetPosition()) .. ' which is ' .. M27Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber]))
                end
                ...
                if not (oUnit.Dead) and oUnit.GetUnitId then
                    ...
                    if iDistanceFromACUToStart > aiBrain[refiNearToACUThreshold] then
                        ...
                        if M27Utilities.IsTableEmpty(tFriendlyPriorityDefence) == false then
                            for iPriorityDefence, oPriorityDefence in tFriendlyPriorityDefence do
                                iCurDistanceToStart = math.min(iCurDistanceToStart, 75 + M27Utilities.GetDistanceBetweenPositions(tUnitCurPosition, oPriorityDefence:GetPosition()))
                    ...
                    if bDebugMessages == true then
                        LOG(sFunctionRef .. ': iCurTargetModDistanceFromStart=' .. iCurTargetModDistanceFromStart .. '; iMapMidpointDistance=' .. iMapMidpointDistance)
                    end
                    if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyAirDominance then
                                    ...
                                    if iCurDistanceToStart <= iMapMidpointDistance then
                                        ...
                                        if iCurTargetModDistanceFromStart <= aiBrain[M27Overseer.refiModDistFromStartNearestOutstandingThreat] then
                                            ...
                                            if M27Utilities.IsTableEmpty(tFriendlyGroundUnits) == false then
                                                ...
                                                bCloseEnoughToConsider = true
                                            elseif bDebugMessages == true then
                                                LOG(sFunctionRef .. ': No friendly units near ' .. repr(tUnitCurPosition))
                                            end
                                        end
                                    end
                    ...
                    while iRemainingMassThreat > 0 and bAbortAsNoMoreAirAA == false do
                        ...
                        if iCurLoopCount < iThresholdToDisableDistanceCheck then
                            ...
                            for iAirAA, oAirAA in aiBrain[reftAvailableAirAA] do
                                if iAirAA > iMaxAirAA then
                                    if bDebugMessages == true then
                                        LOG(sFunctionRef .. ': Considering more AA than want to this cycle so aborting')
                                    end
                        ...
                        if oClosestAirAA then
                            if bDebugMessages == true then
                                ...
                                LOG(sFunctionRef .. ': Clearing commands for closest airAA and then telling it to attack oCurTarget, ClosestAirAA=' .. oClosestAirAA.UnitId .. '; Unique ID=' .. iLifetimeCount .. '; iClosestAirAARef=' .. iClosestAirAARef .. 'ClosestAA Pos=' .. repr(oClosestAirAA:GetPosition()))
                            end
...
function SetupAirOverseer(aiBrain)
    ...
    if bDebugMessages == true then
        LOG(sFunctionRef .. ': iMapMaxSegmentX=' .. iMapMaxSegmentX .. '; iMapMaxSegmentZ=' .. iMapMaxSegmentZ .. '; rPlayableArea=' .. repr(rPlayableArea) .. '; iAirSegmentSize=' .. iAirSegmentSize)
    end
...
function UpdateMexScoutingPriorities(aiBrain)
    ...
    if M27Utilities.IsTableEmpty(aiBrain[M27MapInfo.reftSortedMexesInOriginalGroup]) == true then
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef .. ': Sorted mex count=' .. table.getn(aiBrain[M27MapInfo.reftSortedMexesInOriginalGroup]) .. '; full list of sorted mex locations=' .. repr(aiBrain[M27MapInfo.reftSortedMexesInOriginalGroup]))
        end
        ...
        for iCurTable = 1, iTablesToCycleThrough do
            ...
            for iMex, tMexLocation in tMexesToCycleThrough do
                ...
                if M27Conditions.IsMexUnclaimed(aiBrain, tMexLocation, true, false, false) == true then
                    ...
                    if bDebugMessages == true then
                        LOG(sFunctionRef .. ': Mex location is either unclaimed or has enemy mex on it, location=' .. repr(tMexLocation) .. '; iPriorityCount=' .. iPriorityCount)
                    end
                    ...
                    if bDebugMessages == true then
                        LOG(sFunctionRef .. ': Mex location is already built on by us or queued, location=' .. repr(tMexLocation))
                    end
    ...
    if oTarget then
        ...
        if iOrderType == refOrderMove then
            ...
            if not (iOrderType == oNovax[refiLastIssuedOrderType] and oNovax[reftLastIssuedOrderLocation] and M27Utilities.GetDistanceBetweenPositions(oNovax[reftLastIssuedOrderLocation], oTarget:GetPosition()) <= 8) then
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': Issuing new order to novax, telling it to move to ' .. repr(oNovax[reftLastIssuedOrderLocation]))
                end
...
function ExperimentalGunshipCoreTargetLoop(aiBrain, oUnit, bIsCzar)
    ...
    if bDebugMessages == true then
        if M27UnitInfo.IsUnitValid(oUnit[refoLastUnitTarget]) then
            ...
            LOG(sFunctionRef .. ': Last target was a location or nil =' .. repr((oUnit[reftLastLocationTarget] or { 'nil' })))
        end
    ...
    if not (oAttackTarget) and M27Utilities.IsTableEmpty(tLocationToMoveTo) then
        ...
        if M27UnitInfo.IsUnitValid(aiBrain[M27Overseer.refoLastNearestACU]) and not (IsTargetInDeepWater(aiBrain[M27Overseer.refoLastNearestACU])) and not (M27Logic.IsTargetUnderShield(aiBrain, aiBrain[M27Overseer.refoLastNearestACU], 14000, false, false, true)) and GetSegmentFailedAttempts(aiBrain[M27Overseer.refoLastNearestACU]:GetPosition()) < iMaxPrevTargets then
            ...
            if M27Utilities.CanSeeUnit(aiBrain, aiBrain[M27Overseer.refoLastNearestACU], true) or aiBrain[M27Overseer.refoLastNearestACU] == oUnit[refoLastUnitTarget] then
                oAttackTarget = aiBrain[M27Overseer.refoLastNearestACU]
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': Last ACU position based on unit=' .. repr(oAttackTarget:GetPosition()))
                end
            else
                tLocationToMoveTo = aiBrain[M27Overseer.reftLastNearestACU]
                if bDebugMessages == true then
                    LOG(sFunctionRef .. ': Last ACU position based on location=' .. repr(tLocationToMoveTo))
                end
            end
        else
    ...
    if oAttackTarget or M27Utilities.IsTableEmpty(tLocationToMoveTo) == false then
        ...
        if oAttackTarget then
            ...
            if (not (oAttackTarget == oUnit[refoLastUnitTarget]) and (not (bIsCzar) or M27Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), oAttackTarget:GetPosition()) >= 2)) or M27Logic.IsUnitIdle(oUnit) then
                ...
                if bIsCzar then
                    ...
                    if M27UnitInfo.IsUnitUnderwater(oAttackTarget) then
                        ...
                        if bDebugMessages == true then
                            LOG(sFunctionRef .. ': Given attack ground order to try and hit unit ' .. oAttackTarget.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oAttackTarget) .. ' at position ' .. repr(oAttackTarget:GetPosition()))
                        end
                    else
                    ...
                    if bDebugMessages == true then
                        LOG(sFunctionRef .. ': Given attack unit order to ' .. oAttackTarget.UnitId .. M27UnitInfo.GetUnitLifetimeCount(oAttackTarget) .. ' at position ' .. repr(oAttackTarget:GetPosition()))
                    end
                end
            ...
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': Unit to attack position=' .. repr(oAttackTarget:GetPosition()))
            end
        else
            if bDebugMessages == true then
                LOG(sFunctionRef .. ': tLocationToMoveTo=' .. repr(tLocationToMoveTo) .. '; oUnit[reftLastLocationTarget]=' .. repr(oUnit[reftLastLocationTarget] or { 'nil' }))
            end
