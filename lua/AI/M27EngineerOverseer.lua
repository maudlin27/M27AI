...
function ProcessingEngineerActionForNearbyEnemies(aiBrain, oEngineer)
    ...
    tNearbyUnits = aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryPD, tEngPosition, math.min(aiBrain[M27Overseer.refiSearchRangeForEnemyStructures], 73), 'Enemy')
...
function IssuePlateauSpareEngineerAction(aiBrain, oEngineer)
    ...
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerStart)
    --if GetEngineerUniqueCount(oEngineer) == 61 and GetGameTimeSeconds() >= 2500 then bDebugMessages = true end
                ...
                if not(oHive:IsUnitState('Upgrading')) then
                    ...
                    if not(iCurOrder) then
                        ...
                        if iRefreshCount == 0 then
                            ...
                            if not(iLastOrder) then
                                oHive:SetPaused(false)
                                oHive[M27UnitInfo.refbPaused] = false
                            end

                            IssueClearCommands({oHive})
...
function IssueSpareEngineerAction(aiBrain, oEngineer)
    ...
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerStart)
    --if GetEngineerUniqueCount(oEngineer) == 169 and GetGameTimeSeconds() >= 2580 then bDebugMessages = true end
                    ...
                    for iUnit, oUnit in tNearbyExperimentalSubset do
                        if oUnit:GetFractionComplete() >= 0.03 and oUnit:GetFractionComplete() < 1 then
                            ...
                            if iBasePathingGroup == M27MapInfo.GetSegmentGroupOfLocation(M27UnitInfo.refPathingTypeAmphibious, oUnit:GetPosition()) then
                                ...
                                if oUnit:GetAIBrain():GetArmyIndex() == aiBrain:GetArmyIndex() or (oUnit:GetFractionComplete() < 0.8 and (oUnit:GetBlueprint().Economy.BuildCostMass >= 65000 or M27Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber]) <= 120)) then
                ...
                if iFactionIndex == M27UnitInfo.refFactionUEF then
                    ...
                    if not(iCategoryRef) then
                        ...
                        if iExistingLandExperimentals <= 2 and aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithLand] and aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] <= 600 and iExistingLandExperimentals <= 1 and (iEnemyT2ArtiCount <= (iExistingLandExperimentals + 1) * 2 or table.getn(aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryFixedT2Arti, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], 725, 'Enemy')) <= (iExistingLandExperimentals + 1) * 2) and (M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryLandExperimental, 3) or not(M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryFixedT3Arti + M27UnitInfo.refCategoryExperimentalArti, 1))) then
                            ...
                            if bTargetsForT3Arti and aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] <= 820 and ((not(aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious]) and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 10) or (aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 15 and (aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] >= 325 or not(M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryLandExperimental, 4))))) and aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryT3Mex) >= math.max(1, table.getn(M27MapInfo.tResourceNearStart[M27Utilities.GetAIBrainArmyNumber(aiBrain)][1])) then
                                iCategoryRef = refiExperimentalT3Arti
                            end
                        end
                        if not(iCategoryRef) then
    ...
    if iActionRefToAssign == refActionUpgradeBuilding or iActionRefToAssign == refActionAssistSMD or iActionRefToAssign == refActionAssistTML or iActionRefToAssign == refActionAssistNuke or iActionRefToAssign == refActionAssistAirFactory or iActionRefToAssign == refActionUpgradeHQ or iActionRefToAssign == refActionAssistShield then
        ...
        elseif iActionRefToAssign == refActionAssistShield then
            ...
            if M27Utilities.IsTableEmpty(aiBrain[reftPriorityShieldsToAssist]) == false then --redundancy
                ...
                if M27Utilities.IsTableEmpty(aiBrain[reftEngineerAssignmentsByActionRef][iActionRefToAssign]) == false then iMaxEngisWanted = math.min(30, math.max(4, math.ceil(table.getsize(aiBrain[reftEngineerAssignmentsByActionRef][iActionRefToAssign]) / table.getsize(aiBrain[reftPriorityShieldsToAssist])) + 1)) end
            ...
            for iFirebaseRef, tFirebaseStringKeyUnits in aiBrain[reftFirebaseUnitsByFirebaseRef] do
                ...
                if not(bWantFortification) then
                    ...
                    if not(bWantFortification) then
                        ...
                        if aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= (3 + iT2Arti) and (iEnemyT2PlusIndirect > 0 or iMassInvested >= 7000 or table.getsize(tT2PlusPD) >= 5 or aiBrain[M27Overseer.refiTotalEnemyLongRangeThreat] >= 1000) then
                            local iArtiWanted = 1
...
function MonitorShieldHealth(aiBrain, oShield)
    ...
    if M27Utilities.IsTableEmpty(oShield[reftAssistingEngineers]) == false and not(oShield[refbActiveShieldHealthChecker]) then
        ...
        while M27UnitInfo.IsUnitValid(oShield) do
            ...
            if M27Utilities.IsTableEmpty(aiBrain[M27Overseer.reftEnemyArtiAndExpStructure]) == false then
                if aiBrain:GetEconomyStoredRatio('MASS') >= 0.95 and aiBrain:GetEconomyStoredRatio('ENERGY') >= 1 and aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] >= 100 then
...
function ReleaseEngineerIfHaveEmergency(aiBrain)
        ...
        if iT3Engineers >= 10 then
            ...
            if aiBrain[M27AirOverseer.refbHaveAirControl] == false and aiBrain[M27AirOverseer.refiEnemyAirToGroundThreat] >= 15000 and aiBrain[M27AirOverseer.refiNearestEnemyAirThreatActualDist] <= math.min(175, aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] * 0.5) then
                ...
                if M27UnitInfo.IsUnitValid(aiBrain[M27Overseer.refoNearestThreat]) and aiBrain[M27Overseer.refiModDistFromStartNearestThreat] <= math.max(100, aiBrain[M27AirOverseer.refiBomberDefenceCriticalThreatDistance]) and EntityCategoryContains(M27UnitInfo.refCategoryLandCombat + M27UnitInfo.refCategoryIndirect - categories.TECH1, aiBrain[M27Overseer.refoNearestThreat].UnitId) then
                    ...
                    --Nearby experimental land threat?

                    local iEmergencyRange = math.min(175, aiBrain[M27Overseer.refiDistanceToNearestEnemyBase]*0.5)
                    local iFatboyMod
                    if M27Utilities.IsTableEmpty(aiBrain[M27Overseer.reftEnemyLandExperimentals]) == false then
                        for iUnit, oUnit in aiBrain[M27Overseer.reftEnemyLandExperimentals] do
                            if EntityCategoryContains(M27UnitInfo.refCategoryFatboy, aiBrain[M27Overseer.reftEnemyLandExperimentals]) then iFatboyMod = 50 else iFatboyMod = 0 end
                            if M27Utilities.GetDistanceBetweenPositions(oUnit:GetPosition(), M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber]) <= iEmergencyRange + iFatboyMod then
                                bReleaseEngineer = true
                                break
                            end
                        end
            ...
            if bReleaseEngineer then
                ...
                local tEngineers = aiBrain:GetListOfUnits(refCategoryEngineer * categories.TECH3, false, true)
                local oHighestConditionEngineer
                if M27Utilities.IsTableEmpty(tEngineers) == false then
                    for iEngineer, oEngineer in tEngineers do
                        if oEngineer:GetFractionComplete() >= 1 and not(oEngineer[refbPrimaryBuilder]) and oEngineer[refiEngineerConditionNumber] > iHighestConditionActive and M27Utilities.GetDistanceBetweenPositions(oEngineer:GetPosition(), M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber]) <= 60 then
                            iHighestConditionActive = oEngineer[refiEngineerConditionNumber]
                            oHighestConditionEngineer = oEngineer
                        end
                    end
                end
                if oHighestConditionEngineer then
                    IssueClearCommands({oHighestConditionEngineer})
...
function ReassignEngineers(aiBrain, bOnlyReassignIdle, tEngineersToReassign)
    ...
    if M27Utilities.IsTableEmpty(aiBrain[reftEngineerAssignmentsByActionRef]) == false then
        for iActionRef, oRecordedEngineer in aiBrain[reftEngineerAssignmentsByActionRef] do
            ...
            if aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] >= 50000 then bHaveLowPower = false bWantMorePower = false bHaveVeryLowPower = false end
                ...
                if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyACUKill and aiBrain[M27Overseer.refbIncludeACUInAllOutAttack] then
                        ...
                        if not(iActionToAssign) and aiBrain[M27Overseer.refbT2NavyNearOurBase] then
                            ...
                            if M27Utilities.IsTableEmpty(tEnemyMissileNavy) == false then
                                ...
                                if M27Utilities.IsTableEmpty(tTMDOwned) or table.getn(tTMDOwned) < iTMDWanted and aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryTMD) < iTMDWanted * 2 then
                                    ...
                                    iMinEngiTechLevelWanted = 2
                                    iMaxEngisWanted = 3
                                    if not(bHaveLowMass) and not(bHaveVeryLowPower) then iMaxEngisWanted = 5 end
                                end
                            end
                        end

                    elseif iCurrentConditionToTry == 4 then --Emergency PD and (if turtling) firebase fortification; also emergency T2 Arti
                        --Dont want to get this too early even if threats detected, so require at least 15 mass income per tick
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering if we want emergency PD or Arti. Gross mass income='..aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome]..'; bHaveLowPower='..tostring(bHaveLowPower)..'; Gross energy income='..aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome]..'; Is table of existing engineers assigned to build PD empty='..tostring(M27Utilities.IsTableEmpty(aiBrain[reftEngineerAssignmentsByActionRef][refActionBuildEmergencyPD]))..'; Land segment group of nearest enemy='..M27MapInfo.GetSegmentGroupOfLocation(M27UnitInfo.refPathingTypeLand, aiBrain[M27Overseer.reftLocationFromStartNearestThreat])..'; Land segment group of base='..M27MapInfo.GetSegmentGroupOfLocation(M27UnitInfo.refPathingTypeLand, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber])) end
                        if aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 1.5 and (not(bHaveLowPower) or aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] >= 65) then
                            ...
                            if M27Utilities.IsTableEmpty(aiBrain[reftEngineerAssignmentsByActionRef][refActionBuildEmergencyPD]) == false then
                                ...
                                if M27MapInfo.GetSegmentGroupOfLocation(M27UnitInfo.refPathingTypeLand, aiBrain[M27Overseer.reftLocationFromStartNearestThreat]) == M27MapInfo.GetSegmentGroupOfLocation(M27UnitInfo.refPathingTypeLand, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber]) then
                                    ...
                                    --Increase search range if we lack air control
                            ...
                            if bDebugMessages == true then LOG(sFunctionRef..': Finished checking if want emergency PD. iActionToAssign='..(iActionToAssign or 'nil')..'; Gross income='..aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome]) end
                        end
                    ...
                    elseif iCurrentConditionToTry == 5 then
                        if not(iActionToAssign) and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 5 then
                            ...
                            if aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious] and M27Utilities.IsTableEmpty(aiBrain[M27Overseer.reftEnemyLandExperimentals]) == false and M27Utilities.IsTableEmpty(EntityCategoryFilterDown(M27UnitInfo.refCategoryFatboy, aiBrain[M27Overseer.reftEnemyLandExperimentals])) == false then bEnemyHasFatboy = true end
                            if bDebugMessages == true then LOG(sFunctionRef..': bEnemyHasFatboy='..tostring(bEnemyHasFatboy)..'; Torps wanted='..aiBrain[M27AirOverseer.refiTorpBombersWanted]) end
                            if not(bEnemyHasFatboy) then
                                --Do the enemy have nearby indirect fire?
                                local tNearbyIndirectOrT2Structure = aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryLongRangeMobile + M27UnitInfo.refCategoryStructure - categories.TECH1, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], 150, 'Enemy')
                                if M27Utilities.IsTableEmpty(tNearbyIndirectOrT2Structure) == false then bEnemyHasNearbyT2StructureOrLongRange = true end
                            end
                            if bEnemyHasFatboy or bEnemyHasNearbyT2StructureOrLongRange or aiBrain[M27AirOverseer.refiTorpBombersWanted] >= 4 then
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': Deciding if need emergency T2 Arti. Torp bombers wanted='..aiBrain[M27AirOverseer.refiTorpBombersWanted]..'; Is nearby enemy navy empty='..tostring(M27Utilities.IsTableEmpty(tNearbyEnemyNavy))..'; bEnemyHasFatboy='..tostring(bEnemyHasFatboy)) end
                                if bEnemyHasFatboy or bEnemyHasNearbyT2StructureOrLongRange or (M27Utilities.IsTableEmpty(tNearbyEnemyNavy) == false and (table.getn(tNearbyEnemyNavy) >= 3 or M27Utilities.IsTableEmpty(EntityCategoryFilterDown(M27UnitInfo.refCategoryCruiserCarrier, tNearbyEnemyNavy)) == false)) then
                                    ...
                                    if bDebugMessages == true then LOG(sFunctionRef..': Nearby fatboy or naval threat, will check how much navy; iNearbyT2Arti='..iNearbyT2Arti..'; bEnemyHasFatboy='..tostring(bEnemyHasFatboy)) end
                                    if iNearbyT2Arti == 0 or (not(bEnemyHasFatboy or bEnemyHasNearbyT2StructureOrLongRange) and table.getn(tNearbyEnemyNavy) / 3 > iNearbyT2Arti) or (iNearbyT2Arti <= 7 and (bEnemyHasFatboy or bEnemyHasNearbyT2StructureOrLongRange) and M27Utilities.IsTableEmpty(aiBrain[reftFirebaseUnitsByFirebaseRef])) then
                                        iActionToAssign = refActionBuildEmergencyArti
                                        iMinEngiTechLevelWanted = 2
                                        iMaxEngisWanted = 8
                                        if bHaveLowMass then iMaxEngisWanted = 3 end
                                        if bDebugMessages == true then LOG(sFunctionRef..': Will build arti by base') end
                                    elseif iNearbyT2Arti <= 7 and (bEnemyHasFatboy or (bEnemyHasNearbyT2StructureOrLongRange and iNearbyT2Arti <= 3)) then
                                        ...
                                        if aiBrain[refiFirebaseBeingFortified] then
                                            ...
                                            if bBuildAtFirebase then
                                                ...
                                                if bDebugMessages == true then LOG(sFunctionRef..': Will build emergency Arti at '..repru(tExistingLocationsToPickFrom)) end
                                            end
                                        end
                                    end
                                end
                    ...
                    elseif iCurrentConditionToTry == 7 then  --want 2 engis claiming mexes for first 5m of game (and for initial 2 engis to keep building mexes)
                    ...
                    elseif iCurrentConditionToTry == 8 then --Get reclaim if low on mass
                    ...
                    elseif iCurrentConditionToTry == 9 then --Want 2-5 factories as a high priority (higher than power)
                    ...
                    elseif iCurrentConditionToTry == 11 then --Naval threat near our base - assist air factory
                        if aiBrain[M27Overseer.refbT2NavyNearOurBase] then
                            ...
                            if iAirFactories > 0 then
                                ...
                                if bHaveVeryLowPower then iMaxEngisWanted = 4
                                elseif bHaveLowPower then iMaxEngisWanted = 8
                                elseif bHaveLowMass then iMaxEngisWanted = 10
                                else iMaxEngisWanted = 15
                                end
                            end
                        end
                    elseif iCurrentConditionToTry == 12 then --Land factories when are getting close to overflowing mass and dont have that many
                        if aiBrain:GetEconomyStoredRatio('MASS') >= 0.3 or aiBrain:GetEconomyStored('MASS') >= 1750 then
                            if iLandFactories == nil then
                                iLandFactories = aiBrain:GetCurrentUnits(refCategoryLandFactory)
                                if iLandFactories == nil then iLandFactories = 0 end
                            end
                            if iLandFactories < math.min(4, aiBrain[M27Overseer.reftiMaxFactoryByType][M27Overseer.refFactoryTypeLand]) then
                                ...
                                if bHaveVeryLowPower then iMaxEngisWanted = 1
                                elseif bHaveLowPower then iMaxEngisWanted = 2 end
                            end
                            if bDebugMessages == true then LOG(sFunctionRef..': Land factory builder if may soon be overflowing: iLandFactories='..iLandFactories..'; iActionToAssign='..(iActionToAssign or 'nil')..'; bHaveLowPower='..tostring(bHaveLowPower)..'; Max land factories wanted='..aiBrain[M27Overseer.reftiMaxFactoryByType][M27Overseer.refFactoryTypeLand]) end
                        end
                    elseif iCurrentConditionToTry == 13 then --Non-early game want 1 engi getting unclaimed mexes as a higher priority
                    ...
                    elseif iCurrentConditionToTry == 14 then --Low power action part 1; also T3 power adjacency logic (as want this built as high priority  ahead of normal power)
                    ...
                    elseif iCurrentConditionToTry == 15 then --High priority energy storage
                        ...
                        if aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] >= 1 or iEnergyStoredRatio == 1 then
                            if M27Utilities.IsACU(M27Utilities.GetACU(aiBrain)) then
                                ...
                                if (M27Utilities.GetACU(aiBrain).PlatoonHandle[M27PlatoonUtilities.refiEnemiesInRange] or 0) >= 3 or M27UnitInfo.GetUnitHealthPercent(M27Utilities.GetACU(aiBrain)) < 0.9 or (aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious] and aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] >= 100) or aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] >= 250 then
                                    if iEnergyStoredRatio == nil then iEnergyStoredRatio = aiBrain:GetEconomyStoredRatio('ENERGY') end
                                    if iEnergyStoredRatio > 0 then
                                        if iEnergyStorageMax == nil then iEnergyStorageMax = iEnergyStored / iEnergyStoredRatio end
                                        if iEnergyStorageMax < 9000 or (aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] >= 75 and aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] >= 10 and iEnergyStorageMax < math.min(67000, 4 * aiBrain[M27Overseer.refiHighestEnemyGroundUnitHealth] / 0.9)) then
                                            iActionToAssign = refActionBuildEnergyStorage
                                ...
                                if iCurStorageUnits == 0 or iCurStorageUnits < math.min(6, math.floor(aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] * 0.01)) then
                                    iActionToAssign = refActionBuildEnergyStorage
                                    iMaxEngisWanted = 4
                                end
                            end
                    ...
                    elseif iCurrentConditionToTry == 16 then
                        ...
                        if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyTurtle and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 4 then
                            RefreshListOfFirebases(aiBrain)
                    ...
                    elseif iCurrentConditionToTry == 18 then --SMD
                        ...
                        if iHighestFactoryOrEngineerTechAvailable >= 3 and (M27Utilities.IsTableEmpty(aiBrain[M27Overseer.reftEnemyNukeLaunchers]) == false or aiBrain[M27Overseer.refbEnemyFiredNuke] or (not(bHaveLowMass)) and not(bHaveLowPower) and aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] >= 300 and (aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 15 or (aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 12 and M27Conditions.GetLifetimeBuildCount(aiBrain, M27UnitInfo.refCategoryExperimentalLevel) >= 2))) then
                            ...
                            if iSMDsWeHave < iEnemyNukes then
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': Have fewer SMDs than enemy has nukes') end
                    ...
                    elseif iCurrentConditionToTry == 20 then --Omni to protect from cloaked ACU/SACU, or if have ok resources and either enemy has large air threat or we are Sera/Aeon with skirmishers needing intel
                        if iHighestFactoryOrEngineerTechAvailable >= 3 and (aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] >= 350 or aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] >= 1000) and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 5 then
                            ...
                            if not(bWantEmergencyOmni) then
                                ...
                                if bHaveLowMass == false and not(aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyProtectACU) and aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] >= 175 then
                            ...
                            if bWantEmergencyOmni then
                                ...
                                if M27Utilities.IsTableEmpty(aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryT3Radar, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], iMaxFriendlyOmniDistance, 'Ally')) then
                                    ...
                                    if not(aiBrain[M27Overseer.refbCloakedEnemyACU]) and aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] <= 300 and bHaveLowPower then iMaxEngisWanted = 2 end
                                end
                            end
                        end
                    elseif iCurrentConditionToTry == 21 then --Assist air fac if ACU needs protecting, or has taken recent torpedo damage and is underwater, or we have enemies near our base
                        if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyProtectACU or (aiBrain[M27AirOverseer.refiAirAANeeded] < 1 and aiBrain[M27Overseer.refiModDistFromStartNearestThreat] <= math.min(150, aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] * 0.4)) then
                            if iAirFactories == nil then
                                iAirFactories = aiBrain:GetCurrentUnits(refCategoryAirFactory)
                                if iAirFactories == nil then iAirFactories = 0 end
                            end
                    ...
                    elseif iCurrentConditionToTry == 22 then --Shield if needed for SML/SMD/T3 Arti as high priority
                        if not(bHaveVeryLowPower) and (aiBrain[M27Overseer.refbDefendAgainstArti] or (not(bHaveLowPower) and aiBrain:GetEconomyStored('MASS') > 0)) then
                            ...
                            if bDebugMessages == true then LOG(sFunctionRef..': Is table of units wanting fixed shield empty='..tostring(M27Utilities.IsTableEmpty(aiBrain[reftUnitsWantingFixedShield]))) end
                            if M27Utilities.IsTableEmpty(aiBrain[reftUnitsWantingFixedShield]) == false and M27Utilities.IsTableEmpty(EntityCategoryFilterDown(M27UnitInfo.refCategorySMD + M27UnitInfo.refCategorySML + M27UnitInfo.refCategoryFixedT3Arti + M27UnitInfo.refCategoryExperimentalArti * categories.STRUCTURE, aiBrain[reftUnitsWantingFixedShield])) == false then
                                ...
                                iMinEngiTechLevelWanted = 3
                                if bDebugMessages == true then LOG(sFunctionRef..': Is assignments for build shield empty='..tostring(M27Utilities.IsTableEmpty(aiBrain[reftEngineerAssignmentsByActionRef][refActionBuildShield]))..'; Mass stored='..aiBrain:GetEconomyStored('MASS')..'; bHaveLowPower='..tostring(bHaveLowPower)) end
                    ...
                    elseif iCurrentConditionToTry == 23 then --Assist shields to defend against arti
                        if not(bHaveLowPower) and aiBrain[M27Overseer.refbDefendAgainstArti] then
                            ...
                            if M27Utilities.IsTableEmpty(aiBrain[reftPriorityShieldsToAssist]) == false then
                                ...
                                if M27Utilities.IsTableEmpty(aiBrain[M27Overseer.reftEnemyArtiAndExpStructure]) == false then
                                    for iUnit, oUnit in aiBrain[M27Overseer.reftEnemyArtiAndExpStructure] do
                                        if M27UnitInfo.IsUnitValid(oUnit) then
                                            if EntityCategoryContains(M27UnitInfo.refCategoryNovaxCentre, oUnit.UnitId) then iEnemyT3ArtiValue = iEnemyT3ArtiValue + 0.5
                                            elseif EntityCategoryContains(M27UnitInfo.refCategoryFixedT3Arti + M27UnitInfo.refCategoryExperimentalArti, oUnit.UnitId) then iEnemyT3ArtiValue = iEnemyT3ArtiValue + 1
                                            end
                                        end
                                    end
                                end
                    ...
                    elseif iCurrentConditionToTry == 24 then --Shield for T2 arti if are turtling
                        if not(bHaveLowPower) and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 6 and aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyTurtle then
                            ...
                            if M27Utilities.IsTableEmpty(aiBrain[reftUnitsWantingFixedShield]) == false and M27Utilities.IsTableEmpty(EntityCategoryFilterDown(M27UnitInfo.refCategoryFixedT2Arti, aiBrain[reftUnitsWantingFixedShield])) == false then
                                ...
                                if bHaveLowMass then iMaxEngisWanted = 3 end
                                iMinEngiTechLevelWanted = 2
                                if aiBrain[M27Overseer.refbDefendAgainstArti] or aiBrain[M27Overseer.refbCloseToUnitCap] then iMinEngiTechLevelWanted = 3 end
                            end
                        end
                    elseif iCurrentConditionToTry == 25 then --Initial engi for experimental (to reduce risk we spend a while looking for somewhere to build)
                        if iMassStoredRatio == nil then iMassStoredRatio = aiBrain:GetEconomyStoredRatio('MASS') end
                    ...
                    elseif iCurrentConditionToTry == 26 then --Engis for transport
                        if bDebugMessages == true then LOG(sFunctionRef..': Is table of transports waiting for engineers empty='..tostring(M27Utilities.IsTableEmpty(aiBrain[M27Transport.reftTransportsWaitingForEngi]))) end
                        if M27Utilities.IsTableEmpty(aiBrain[M27Transport.reftTransportsWaitingForEngi]) == false then
                            iActionToAssign = refActionLoadOnTransport
                            iMaxEngisWanted = aiBrain[M27Transport.refiEngineersWantedForTransports]
                    ...
                    elseif iCurrentConditionToTry == 27 then --Nuke assist
                    ...
                    elseif iCurrentConditionToTry == 28 then --Unclaimed mexes within defender coverage
                    ...
                    elseif iCurrentConditionToTry == 29 then --T1/T2 radar if enemy may have significant air threat or high tech units, and radar if we have skirmishers needing better intel
                        if bHaveLowPower == false then
                            ...
                            if iCurRadarCount == 0 then
                                if iNetCurEnergyIncome > 5 and iEnergyStored >= 2000 and not(bHaveLowMass) and (aiBrain[M27AirOverseer.refiHighestEnemyAirThreat] >= 50 or aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 3 or aiBrain:GetEconomyStored('MASS') >= 400 or M27Utilities.IsTableEmpty(aiBrain[M27PlatoonUtilities.reftSkirmisherPlatoonWantingIntel]) == false) then
                    ...
                    elseif iCurrentConditionToTry == 30 then --Hive near firebase T3 shield that has reclaim nearby
                    ...
                    elseif iCurrentConditionToTry == 31 then --Hydro within our defence coverage?'
                    ...
                    elseif iCurrentConditionToTry == 32 then --Static AA due to losing air control even if no nearby air threats
                        ...
                        if not(aiBrain[M27AirOverseer.refbHaveAirControl]) and (aiBrain[M27Overseer.refbEmergencyMAANeeded] or aiBrain[M27AirOverseer.refiHighestEnemyAirThreat] > 1000) then
                            ...
                            if iStaticAAWanted > 0 then
                                ...
                                if iExistingAA < iStaticAAWanted then
                                    iActionToAssign = refActionBuildAA
                                    iMinEngiTechLevelWanted = iHighestFactoryOrEngineerTechAvailable
                                    if bHaveLowPower then iMaxEngisWanted = 1
                                    elseif bHaveLowMass then iMaxEngisWanted = 2
                                    else iMaxEngisWanted = math.min((iStaticAAWanted - iExistingAA) * 3, 6)
                                    end
                                end
                            end
                        end
                    elseif iCurrentConditionToTry == 33 then --Lower power action part 2 - want enough power to support guncom
                    ...
                    elseif iCurrentConditionToTry == 34 then --Mass storage around T2/T3 mexes
                    ...
                    elseif iCurrentConditionToTry == 35 then --TML if think enough targets
                                ...
                                if M27Utilities.IsTableEmpty(aiBrain[reftEngineerAssignmentsByActionRef][refActionBuildTML]) then
                                    ...
                                    for iTML, oTML in tFriendlyTML do
                                        if oTML.GetTacticalSiloAmmoCount and oTML:GetTacticalSiloAmmoCount() == 0 and not(oTML:IsPaused()) then
                                            ...
                                            iMaxEngisWanted = 3
                                            if bHaveLowMass then iMaxEngisWanted = 1
                                            elseif oTML.Sync.totalMassKilled >= 1000 then iMaxEngisWanted = 5
                                            end
                                            break
                                        end
                                    end
                    ...
                    elseif iCurrentConditionToTry == 36 then --Higher priority air staging for if we have lots of air units wanting refueling
                    ...
                    elseif iCurrentConditionToTry == 37 then --Fortify firebase
                    ...
                    elseif iCurrentConditionToTry == 38 then --T3 mexes in place of existing T2 mexes
                        if iHighestFactoryOrEngineerTechAvailable >= 3 then
                    ...
                    elseif iCurrentConditionToTry == 39 then --Make sure we have at least 2 T3 land factories if we have significant mass and want more engineers
                    ...
                    elseif iCurrentConditionToTry == 40 then --Queue up an experimental if we have significant mass as it may take time to find the right position
                        if iMassStored == nil then iMassStored = aiBrain:GetEconomyStored('MASS') end
                        if not(bHaveLowPower) and not(bHaveLowMass) and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 7 and iMassStored >= 1000 and (aiBrain[M27EconomyOverseer.refiMassNetBaseIncome] > 1 or aiBrain:GetEconomyStoredRatio('MASS') >= 0.3) and iHighestFactoryOrEngineerTechAvailable >= 3 and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 7 then
                            ...
                            if SafeToBuildExperimental(aiBrain) then
                                ...
                                iMaxEngisWanted = 1
                                if aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 10 and iMassStored >= 2000 then iMaxEngisWanted = math.min(30, math.max(2, math.floor(aiBrain[M27EconomyOverseer.refiMassNetBaseIncome]), math.ceil(iMassStored / 2000))) end
                    ...
                    elseif iCurrentConditionToTry == 41 then --Build factories (or upgrade HQ) if getting too much mass
                    ...
                    elseif iCurrentConditionToTry == 42 then --Air staging if we need one for low fuel air units
                        if bDebugMessages == true then LOG(sFunctionRef..': iCurrentConditionToTry='..iCurrentConditionToTry..': bHaveLowPower='..tostring(bHaveLowPower)..'; aiBrain[M27AirOverseer.refiAirStagingWanted]='..(aiBrain[M27AirOverseer.refiAirStagingWanted] or 'nil')) end
                        ...
                        --end
                    elseif iCurrentConditionToTry == 43 then --Energy storage once have certain level of power
                        if bHaveLowPower == false then
                            if iGrossCurEnergyIncome >= 28 and (aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious] or iGrossCurEnergyIncome >= 100) then
                                ...
                                if iEnergyStoredRatio > 0 then
                                    ...
                                    if not(M27Utilities.IsACU(M27Utilities.GetACU(aiBrain))) then
                                        ...
                                        if aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] >= 75 and aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] >= 15 then iMaxStorageWanted = math.max(iMaxStorageWanted, 4.44445 * aiBrain[M27Overseer.refiHighestEnemyGroundUnitHealth]) end
                    ...
                    elseif iCurrentConditionToTry == 44 then --Get reclaim
                    ...
                    elseif iCurrentConditionToTry == 46 then --Try to get nearest unclaimed mex (i.e. this will only run if are no mexes within defensive area or our side of map):
                        if iAllUnclaimedMexesInPathingGroup == nil then
                            tAllUnclaimedMexesInPathingGroup = GetUnclaimedMexes(aiBrain, sPathing, iPathingGroup, false, false, false)
                            if M27Utilities.IsTableEmpty(tAllUnclaimedMexesInPathingGroup) == false then iAllUnclaimedMexesInPathingGroup = table.getn(tAllUnclaimedMexesInPathingGroup)
                            else iAllUnclaimedMexesInPathingGroup = 0 end
                    ...
                    elseif iCurrentConditionToTry == 47 then --2nd T1 power construction with low priority engineers
                    ...
                    elseif iCurrentConditionToTry == 48 then --Radar near base
                        if bHaveLowPower == false then
                            if iCurRadarCount == nil then iCurRadarCount = aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryRadar) end
                    ...
                    elseif iCurrentConditionToTry == 50 then --reclaim on enemy side of map
                    ...
                    elseif iCurrentConditionToTry == 51 then --Assist air
                            ...
                            if M27Utilities.IsTableEmpty(aiBrain[reftEngineerAssignmentsByActionRef][refActionAssistAirFactory]) == false then
                                for iUniqueRef, tSubtable in aiBrain[reftEngineerAssignmentsByActionRef][refActionAssistAirFactory] do
                                    ...
                                    if M27UnitInfo.IsUnitValid(oEngi) then
                                        IssueClearCommands({oEngi})
                                        ClearEngineerActionTrackers(aiBrain, oEngi, true)
                                        if bDebugMessages == true then LOG(sFunctionRef..': Cleared engineer UC='..GetEngineerUniqueCount(oEngi)..' with LC='..M27UnitInfo.GetUnitLifetimeCount(oEngi)) end
                                    end
                                end
                            end
                        end
                    elseif iCurrentConditionToTry == 52 then --Sonar
                        if bDebugMessages == true then LOG(sFunctionRef..': Checking if we want sonar, aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithLand]='..tostring(aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithLand])..'; aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious]='..tostring(aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious])..'; M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryTorpBomber, 1)='..tostring(M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryTorpBomber, 1))..'; refbNoValidSonarLocations='..tostring((aiBrain[refbNoValidSonarLocations] or false))) end
                        if not(aiBrain[refbNoValidSonarLocations]) then
                    ...
                    elseif iCurrentConditionToTry == 53 then --More reclaim (lower priority locations)
                        if iMassStoredRatio == nil then iMassStoredRatio = aiBrain:GetEconomyStoredRatio('MASS') end
                    ...
                    elseif iCurrentConditionToTry == 54 then --Quantum gateway
                        --if tiAvailableEngineersByTech[3] > 0 then bDebugMessages = true end
                        if bDebugMessages == true then LOG(sFunctionRef..': Dist to nearest enemy base='..aiBrain[M27Overseer.refiDistanceToNearestEnemyBase]..'; bHaveLowPower='..tostring(bHaveLowPower)..'; Mass gross base income='..aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome]..'; Have low mass='..tostring(bHaveLowMass)..'; Mass stored%='..aiBrain:GetEconomyStoredRatio('MASS')..'; Can path with land='..tostring(aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithLand])..'; Lifetime experimental build count='..M27Conditions.GetLifetimeBuildCount(aiBrain, M27UnitInfo.refCategoryExperimentalLevel)) end
                        if not(bHaveLowPower) and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 20 and (aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] >= 700 or (aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] >= 400 and (aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyEcoAndTech or aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyTurtle))) and (not(bHaveLowMass) or aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 25 and aiBrain:GetEconomyStoredRatio('MASS') >= 0.01) and (not(aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithLand]) or aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] >= 850 or M27Conditions.GetLifetimeBuildCount(aiBrain, M27UnitInfo.refCategoryExperimentalLevel) >= 2) then
                            local iCurrentQuantumGateways = aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryQuantumGateway)
                            if iCurrentQuantumGateways < 2 and (iCurrentQuantumGateways == 0 or aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 50) then
                                iActionToAssign = refActionBuildQuantumGateway
                                iMaxEngisWanted = 3
                                if not(bHaveLowMass) then iMaxEngisWanted = 6 end
                                iMinEngiTechLevelWanted = 3
                                if bDebugMessages == true then LOG(sFunctionRef..': WIll try and build a quantum gateway') end
                            end
                        end

                    elseif iCurrentConditionToTry == 55 then --Quantum optics
                        --if tiAvailableEngineersByTech[3] > 0 then bDebugMessages = true end
                        if bDebugMessages == true then LOG(sFunctionRef..': Deciding if we want quantum optics.  aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome]='..aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome]..'; Faction index='..aiBrain:GetFactionIndex()..'; aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome]='..aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome]..'; have low mass='..tostring(M27Conditions.HaveLowMass(aiBrain))..'; Current quantum optics='..aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryQuantumOptics)..'; LC='..M27Conditions.GetLifetimeBuildCount(aiBrain, M27UnitInfo.refCategoryQuantumOptics)..'; LifetimeSpyPlanes='..M27Conditions.GetLifetimeBuildCount(aiBrain, M27UnitInfo.refCategoryAirScout * categories.TECH3)..'; aiBrain[M27AirOverseer.refiExtraAirScoutsWanted]='..aiBrain[M27AirOverseer.refiExtraAirScoutsWanted]) end
                        if aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 15 and aiBrain:GetFactionIndex() == M27UnitInfo.refFactionAeon and aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] >= 250 and M27Conditions.GetLifetimeBuildCount(aiBrain, M27UnitInfo.refCategoryQuantumOptics) < 2 and aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryQuantumOptics) == 0 then
                            ...
                            if (not(M27Conditions.HaveLowMass(aiBrain)) or (iLifetimeSpyPlanes >= 10 and aiBrain[M27AirOverseer.refiExtraAirScoutsWanted] >= 5) and (iLifetimeSpyPlanes >= 14 or (iLifetimeSpyPlanes >= 7 and aiBrain[M27AirOverseer.refiExtraAirScoutsWanted] >= 5) or (aiBrain[M27AirOverseer.refiExtraAirScoutsWanted] >= 10 and aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryAirScout * categories.TECH3) == 0))) then
                                ...
                                iMaxEngisWanted = 3
                                if M27Conditions.HaveLowMass(aiBrain) then iMaxEngisWanted = 2 end
                                iMinEngiTechLevelWanted = 3
                            end
                        end
                    elseif iCurrentConditionToTry == 56 then --Hives
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering if we want to build a hive. Energy stored ratio='..aiBrain:GetEconomyStoredRatio('ENERGY')..'; Net base income='..aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome]..'; gross mass income='..aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome]..'; Economy stored ratio mass='..aiBrain:GetEconomyStoredRatio('MASS')..'; have low mass='..tostring(M27Conditions.HaveLowMass(aiBrain))..'; is table of idle engineers empty='..tostring(M27Utilities.IsTableEmpty(tIdleEngineers))..'; aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome]='..aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome]) end
                        if (aiBrain:GetEconomyStoredRatio('ENERGY') >= 1 or aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] >= 750 or aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] >= 2000) and (aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 20 or (aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 15 and aiBrain:GetEconomyStoredRatio('MASS') >= 0.4)) and not(M27Conditions.HaveLowMass(aiBrain)) then
                            ...
                            if M27Utilities.IsTableEmpty(tIdleEngineers) == false and M27Utilities.IsTableEmpty(EntityCategoryFilterDown(categories.CYBRAN - categories.TECH1, tIdleEngineers)) == false then
                                ...
                                if iCurHives >= 40 or (aiBrain:GetEconomyStoredRatio('MASS') <= 0.6 and iCurHives >= math.max(2, aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] - 15)) then
                                    --Do nothing
                                else
                                    iActionToAssign = refActionBuildHive
                                    iMaxEngisWanted = 4
                    ...
                    elseif iCurrentConditionToTry == 57 then --Experimental if have loads of mass or satisfy other tests; one of these will be if we ahve already started on an experimental
                        ...
                        if iHighestFactoryOrEngineerTechAvailable >= 3 then --at least 250 gross income ignoring reclaim
                            ...
                            if iActionToAssign == refActionBuildExperimental then
                                ...
                                elseif aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] <= 500 then
                                    ...
                                    iMaxEngisWanted = math.min(40, math.max(1, math.floor(aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] / 10), math.floor(aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] / 50)))
                                else
                                    ...
                                    if bHaveLowPower then iMaxEngisWanted = 3
                                        ...
                                        iMaxEngisWanted = math.min(40, math.max(5, math.ceil((aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] - 4) / 2 + aiBrain:GetEconomyStored('MASS') / 800)))
                                    end
                                end
                    ...
                    elseif iCurrentConditionToTry == 60 then
                            ...
                            if (iMassStoredRatio > 0.6 or iMassStored > 12000) then --About to overflow so try to build something
                                ...
                                if iAirFactories < aiBrain[M27Overseer.reftiMaxFactoryByType][M27Overseer.refFactoryTypeAir] then
                                    ...
                                    if bActiveExperimental then
                                        ...
                                        elseif aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] <= 500 then
                                            iMaxEngisWanted = math.min(40, math.max(1, math.min(iMaxEngisWanted, math.floor(aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] / 10), math.floor(aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] / 50))))
                                        end
                                    else
                    ...
                    elseif iCurrentConditionToTry == 62 then --Have a SAM by any fixed shield we own
                        ...
                        iSearchRangeForNearestEngi = 200
                        if iCurrentConditionToTry == 63 then
            ...
            if M27Utilities.IsTableEmpty(tIdleEngineers) == false then
                for iEngineer, oEngineer in tIdleEngineers do
                    if M27UnitInfo.IsUnitValid(oEngineer) and not(oEngineer[refiEngineerCurrentAction]) then
                        IssueSpareEngineerAction(aiBrain, oEngineer)
                        --UpdateEngineerActionTrackers(aiBrain, oEngineer, iActionToAssign, tTargetLocation, bAreAssisting, iConditionNumber, oUnitToAssist, bDontClearExistingTrackers, oUnitToBeDestroyed, iPrimaryEngineerCategoryBuilt)
                        if M27UnitInfo.IsUnitValid(oEngineer) then --need to recheck as may have ctrl-Kd due to the spare action
                            UpdateEngineerActionTrackers(aiBrain, oEngineer, refActionSpare, nil, false, 1000)
                        end
                        if bDebugMessages == true then LOG(sFunctionRef..': Backup spare engi action activated for engineer '..oEngineer.UnitId..M27UnitInfo.GetUnitLifetimeCount(oEngineer)) end
                    end
                end
            end
        end
    ...
    ForkThread(ReleaseEngineerIfHaveEmergency, aiBrain)
