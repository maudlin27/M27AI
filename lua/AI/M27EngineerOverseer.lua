...
function UpdateEngineerActionTrackers(aiBrain, oEngineer, iActionToAssign, tTargetLocation, bAreAssisting, iConditionNumber, oUnitToAssist, bDontClearExistingTrackers, oUnitToBeDestroyed)
    ...
    elseif bAreAssisting == true then
        ...
        if bDebugMessages == true then
            LOG(sFunctionRef..': Recorded engineer with unique ref '..iUniqueRef..'; as a guard for unit with ID= '..oUnitToAssist.UnitId)
...
function ProcessingEngineerActionForNearbyEnemies(aiBrain, oEngineer)
    ...
    --end
        ...
        if bAreNearbyEnemies then
            ...
            if bNearbyMobileEnemies == false or (M27Utilities.IsTableEmpty(tNearbyEnemiesShort) == true and M27Logic.GetCombatThreatRating(aiBrain, tMobileEnemies, true, nil, nil, false, false)) <= 10 then
                ...
                if M27Utilities.IsTableEmpty(tEnemyStructures) == false then
                    ...
                    if bDebugMessages == true then LOG(sFunctionRef..': Will try and reclaim the nearest enemy unit='..oReclaimTarget.UnitId..M27UnitInfo.GetUnitLifetimeCount(oReclaimTarget)) end
                else
            ...
            if M27Utilities.IsTableEmpty(tPossibleEngineers) == false and tNearbyEnemiesLong == tNearbyEnemiesLong then
                ...
                if M27Utilities.IsTableEmpty(tPossibleEngineers) == false and tPossibleEngineers == tNearbyEnemiesLong then
                    ...
                    if bOnlyNearbyEngisOrStructure then
                        oReclaimTarget = M27Utilities.GetNearestUnit(tNearbyUnits, tEngPosition, aiBrain, true)
...
function BuildStructureAtLocation(aiBrain, oEngineer, iCategoryToBuild, iMaxAreaToSearch, iCatToBuildBy, tAlternativePositionToLookFrom, bLookForPartCompleteBuildings, bLookForQueuedBuildings, oUnitToBuildBy, bNeverBuildRandom, iOptionalCategoryForStructureToBuild, bBuildCheapestStructure)
    ...
    if sBlueprintToBuild == nil then
        ...
        LOG('oEngineer='..oEngineer.UnitId..GetEngineerUniqueCount(oEngineer))
        tTargetLocation = nil
    else
...
if bDebugMessages == true then LOG(sFunctionRef..': Found '..iCategoryCount..' categories in common') end--]]
    ...
    if not(iCategoryToBuild) then
        ...
        elseif (iEnemyPDThreat <= 22500 or iFactionIndex == M27UnitInfo.refFactionUEF) and aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious] and aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] <= 500 and M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryLandExperimental, 1) then
            iCategoryToBuild = M27UnitInfo.refCategoryLandExperimental
            ...
            if not(iCategoryToBuild) then
                ...
                if iFactionIndex == M27UnitInfo.refFactionUEF then
                    ...
                    if iFactionIndex == M27UnitInfo.refFactionAeon or iFactionIndex == M27UnitInfo.refFactionCybran or iFactionIndex == M27UnitInfo.refFactionSeraphim then
                        ...
                        local iMaxT3AA = 5
                        local iVulnerableMexesWanted = 4 --For aeon - Czar more versatile for maintaining air control and attritional battles
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': Considering if enemy has low AA; bEnemyHasLowAA='..tostring(bEnemyHasLowAA)..'; aiBrain[M27AirOverseer.refiEnemyMassInGroundAA]='..aiBrain[M27AirOverseer.refiEnemyMassInGroundAA]..'; refiOurMassInAirAA wanted='..aiBrain[M27AirOverseer.refiHighestEnemyAirThreat] * iAirRatioWanted) end
                        if (aiBrain[M27AirOverseer.refiOurMassInAirAA] >= aiBrain[M27AirOverseer.refiHighestEnemyAirThreat] * iAirRatioWanted or aiBrain[M27AirOverseer.refiHighestEnemyAirThreat] <= 5000) and aiBrain[M27AirOverseer.refiAirAANeeded] <= 2 and bEnemyHasLowAA then
                            --Does the enemy have a land experimental within 55% of our base and we dont ahve any experimental air? Then go for air unit
                            if bDebugMessages == true then LOG(sFunctionRef..': We have air control, so will build experimental air unless we already own one') end
                            if bNearbyLandExperimental and aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryAirNonScout * categories.EXPERIMENTAL) <= 0 then
                                iCategoryToBuild = M27UnitInfo.refCategoryAirNonScout * categories.EXPERIMENTAL
                                if bDebugMessages == true then LOG(sFunctionRef..' Will build experimental air') end
                            else
                                --Calculate how many vulnerable enemy mex targets there are - any T2+ mexes with no T3 AA and not under a heavy shield
                                if aiBrain[M27AirOverseer.refiEnemyMassInGroundAA] <= 5000 and aiBrain[M27AirOverseer.refiHighestEnemyAirThreat] <= 2500 and aiBrain[M27AirOverseer.refiAirAANeeded] <= 0 then
                                    --iCategoryToBuild = M27UnitInfo.refCategoryAirNonScout * categories.EXPERIMENTAL
                                    iCategoryToBuild = M27UnitInfo.refCategoryAirNonScout * categories.EXPERIMENTAL
                                    if bDebugMessages == true then
                                        LOG(sFunctionRef..': Enemy has minimal AA so will build air experimental')
                                        if iCategoryToBuild == M27UnitInfo.refCategoryAirNonScout * categories.EXPERIMENTAL then
                                            LOG('Category to build is air experimental') --Note - for some reason this test doesnt work; however the repr confirms it's still looking at air experimentals
                                        else
                                            LOG(repr(EntityCategoryGetUnitList(iCategoryToBuild)))
                                        end
                                    end
                ...
                if not(iCategoryToBuild) then
                    ...
                    if aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious] and (iFactionIndex == M27UnitInfo.refFactionUEF or iEnemyPDThreat <= 22000) then
                        iCategoryToBuild = M27UnitInfo.refCategoryLandExperimental
                    else
                        iCategoryToBuild = M27UnitInfo.refCategoryFixedT3Arti
                    end
    ...
    if M27Utilities.IsTableEmpty(ScenarioInfo.Options.RestrictedCategories) then
        ...
        if M27Utilities.IsTableEmpty(tT3EngisOfOurFaction) == false then
            ...
            if not(M27FactoryOverseer.GetBlueprintsThatCanBuildOfCategory(aiBrain, iCategoryToBuild, oT3EngiOfOurFaction)) then
                ...
                if bDebugMessages == true then LOG(sFunctionRef..': Cant build desired experimental, will get other possibilities') end
                local tPossibleCategory = {M27UnitInfo.refCategoryFixedT3Arti, M27UnitInfo.refCategoryLandExperimental, M27UnitInfo.refCategoryAllAir * categories.EXPERIMENTAL, M27UnitInfo.refCategoryNovaxCentre, M27UnitInfo.refCategorySML, M27UnitInfo.refCategoryAirNonScout * categories.EXPERIMENTAL, M27UnitInfo.refCategoryExperimentalStructure}
    ...
    if bDebugMessages == true then
        ...
        elseif aiBrain[refiLastExperimentalCategory] == M27UnitInfo.refCategoryFixedT3Arti then LOG('Last experimental was T3 arti')
        elseif aiBrain[refiLastExperimentalCategory] == M27UnitInfo.refCategoryNovaxCentre then LOG('Last experimental was novax')
        elseif aiBrain[refiLastExperimentalCategory] == M27UnitInfo.refCategoryAirNonScout * categories.EXPERIMENTAL then LOG('Last experimental was experimental air')
        elseif not(aiBrain[refiLastExperimentalCategory]) then LOG('Last experimental category not specified')
        else
            LOG('Unkown last experimental category; will list out all blueprints that meet this category')
            LOG(repr(EntityCategoryGetUnitList(aiBrain[refiLastExperimentalCategory])))
        end
    end
                        ...
                        if not(iActionToAssign == refActionReclaimArea) and not(iActionToAssign == refActionReclaimTrees) and not(iActionToAssign == refActionBuildShield and not(EntityCategoryContains(categories.MOBILE, oActionTargetObject.UnitId))) then
                            ...
                            elseif iActionToAssign == refActionBuildT3MexOverT2 and M27UnitInfo.IsUnitValid(oActionTargetObject) and EntityCategoryContains(M27UnitInfo.refCategoryMex, oActionTargetObject.UnitId) then
                                ...
                                if oActionTargetObject.GetUnitId then
                                    if bDebugMessages == true then LOG(sFunctionRef..': Telling engineer '..GetEngineerUniqueCount(oEngineer)..'to assist enginner '..GetEngineerUniqueCount(oActionTargetObject)..'; ID='..oActionTargetObject.UnitId..M27UnitInfo.GetUnitLifetimeCount(oActionTargetObject)) end
                        ...
                        if iCategoryToBuild == nil then
                            ...
                            elseif iActionToAssign == refActionBuildTMD then
                                ...
                                if not(tLocationToMoveTowards) then
                                    M27Utilities.ErrorHandler('Backup logic - cant find any enemy TMLs so will just pick enemy base as the location we want to run from; this error is only expected if a TML has been killed recently', true)
                                    tLocationToMoveTowards = M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain)
                                end
...
function GetActionTargetAndObject(aiBrain, iActionRefToAssign, tExistingLocationsToPickFrom, tIdleEngineers, iActionPriority, tsUnitStatesToIgnoreCurrent, iSearchRangeForPrevEngi, iSearchRangeForNearestEngi, bOnlyReassignIdle, bGetInitialEngineer, iMinTechLevelWanted)
    ...
    if bDebugMessages == true then
        ...
        else LOG(sFunctionRef..': oActionObject='..oActionObject.UnitId..M27UnitInfo.GetUnitLifetimeCount(oActionObject))
        end
    end
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerEnd)
        ...
        while bChangesMade do
            ...
            for iUnit, oUnit in aiBrain[reftUnitsWantingFixedShield] do
                ...
                if not(M27UnitInfo.IsUnitValid(oUnit)) or M27Logic.IsTargetUnderShield(aiBrain, oUnit, 3000, false, true, true) then
                    ...
                    table.remove(aiBrain[reftUnitsWantingFixedShield], iUnit)
                    if bDebugMessages == true then LOG(sFunctionRef..': iUnit='..iUnit..'oUnit='..oUnit.UnitId..M27UnitInfo.GetUnitLifetimeCount(oUnit)..': is either invalid or under shield so will remove') end
                    break
                end
            end
        ...
        for iActionRef, oRecordedEngineer in aiBrain[reftEngineerAssignmentsByActionRef] do
            ...
            while iEngineersToConsider >= 0 do --want >= rather than > so get correct calculation of engineers needed
                ...
                if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyACUKill and aiBrain[M27Overseer.refbIncludeACUInAllOutAttack] then
                    ...
                    elseif iCurrentConditionToTry == 12 then --TMD
                        if M27Utilities.IsTableEmpty(aiBrain[reftUnitsWantingTMD]) == false then
