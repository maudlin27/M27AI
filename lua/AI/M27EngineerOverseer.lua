...
refEngineerAssignmentActualLocation = 'ActualLocation' --Used by reftEngineerAssignmentsByActionRef
...
function ClearEngineerActionTrackers(aiBrain, oEngineer, bDontClearUnitThatAreGuarding)
    ...
    if not(aiBrain.M27IsDefeated) and M27Logic.iTimeOfLastBrainAllDefeated < 10 then
        ...
        local tEngiTargetPreClear = {aiBrain[reftEngineerActionsByEngineerRef][iUniqueRef][1][reftActualTargetLocationRef][1], aiBrain[reftEngineerActionsByEngineerRef][iUniqueRef][1][reftActualTargetLocationRef][2], aiBrain[reftEngineerActionsByEngineerRef][iUniqueRef][1][reftActualTargetLocationRef][3]}
        ...
        if iUniqueRef then --Wont have any actions assigned by code if unique ref is nil (since its set by the action tracker)
            if bDebugMessages == true then LOG(sFunctionRef..': iUniqueRef='..iUniqueRef..': LC='..M27UnitInfo.GetUnitLifetimeCount(oEngineer)..': Start of clearing actions. GameTime='..GetGameTimeSeconds()..'; bDontClearUnitThatAreGuarding='..tostring(bDontClearUnitThatAreGuarding)..'; action of engineer that are clearing='..(oEngineer[refiEngineerCurrentAction] or 'nil')) end
...
function ProcessingEngineerActionForNearbyEnemies(aiBrain, oEngineer)
    ...
    if oEngineer and not(oEngineer.Dead) then
        ...
        if bNearbyMobileEnemies == true or bNearbyPD == true or M27Utilities.IsTableEmpty(tNearbyEnemiesLong) == false then
            ...
            if not(bNearbyPD) and M27Utilities.IsTableEmpty(tNearbyEnemiesShort) == true and M27Logic.GetCombatThreatRating(aiBrain, tNearbyEnemiesLong, true, nil, nil, false, false) <= 10 then
...
function FindRandomPlaceToBuild(aiBrain, oBuilder, tStartPosition, sBlueprintToBuild, iSearchSizeMin, iSearchSizeMax, bForcedDebug, iOptionalMaxCycleOverride, bAlreadyRecheckedPathing)
    ...
    if __blueprints[sBlueprintToBuild].Economy.BuildCostMass >= 2000 then
        iCycleAbortCount = 6
        if __blueprints[sBlueprintToBuild].Economy.BuildCostMass >= 6000 then iCycleAbortCount = 7 end
    end
    ...
    while iValidLocationCount == 0 do
        ...
        if iGroupCycleCount > iMaxCycles then
            if iMaxCycles >= iCycleAbortCount then --Sometimes we may be ok with not finding anywhere to build, e.g. for rally points
                ...
                if not(bAlreadyRecheckedPathing) and M27MapInfo.RecheckPathingOfLocation(sPathing, oBuilder, tStartPosition, nil) then
...
function BuildStructureAtLocation(aiBrain, oEngineer, iCategoryToBuild, iMaxAreaToSearch, iCatToBuildBy, tAlternativePositionToLookFrom, bLookForPartCompleteBuildings, bLookForQueuedBuildings, oUnitToBuildBy, bNeverBuildRandom, iOptionalCategoryForStructureToBuild)
    ...
    local bAbortConstruction = false
                                    --GetBlueprintsThatCanBuildOfCategory(aiBrain, iCategoryCondition, oFactory, bGetSlowest, bGetFastest, iOptionalCategoryThatMustBeAbleToBuild)
    local sBlueprintToBuild = M27FactoryOverseer.GetBlueprintsThatCanBuildOfCategory(aiBrain, iCategoryToBuild, oEngineer, false, false, iOptionalCategoryForStructureToBuild)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': sBlueprintToBuild='..(sBlueprintToBuild or 'nil')) end
    ...
    if sBlueprintToBuild == nil then
            ...
            if not(bMexHydroOrStorage) then
                if iCatToBuildBy or oUnitToBuildBy then
                    ...
                    elseif oUnitToBuildBy then
                        sBlueprintBuildBy = oUnitToBuildBy:GetUnitId()
        ...
        if bFindRandomLocation and (bMexHydroOrStorage or EntityCategoryContains(M27UnitInfo.refCategoryFixedShield, sBlueprintToBuild)) and not(bAbortConstruction) then
            ...
            if not(EntityCategoryContains(M27UnitInfo.refCategoryFixedShield, sBlueprintToBuild)) then
                M27Utilities.ErrorHandler('Are trying to build in a random place for am ex/hydro/mass storage - figure out why as this shouldnt trigger; Engineer with UC='..GetEngineerUniqueCount(oEngineer)..'='..oEngineer:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oEngineer)..'; trying to build blueprint='..sBlueprintToBuild)
...
local sUnitID = aiBrain[refoLastExperimentalUnderConstruction]:GetUnitId()
if sUnitID then
...
if bDebugMessages == true then LOG(sFunctionRef..': Found '..iCategoryCount..' categories in common') end--]]
            ...
            if aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] >= 600 and (iFriendlyNukes == 0 or not(aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious]) and aiBrain:GetCurrentUnits(M27UnitInfo.refCategorySML) == 0) then
                ...
                if GetGameTimeSeconds() - (aiBrain[M27AirOverseer.reftAirSegmentTracker][iEnemyBaseSegmentX][iEnemyBaseSegmentZ][M27AirOverseer.refiLastScouted] or 0) <= 100 then
                    ...
                    if M27Utilities.IsTableEmpty(tEnemySMD) then
                        ...
                        if bDebugMessages == true then LOG(sFunctionRef..': Will try and build a nuke') end
                    else
                        ...
                        local tSMLTarget, iSMLDamage = M27Logic.GetBestAOETarget(aiBrain, M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain), iAOE, iDamage, true, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], 0, 5)
                        ...
                        for iStartPoint = 1, table.getn(M27MapInfo.PlayerStartPoints) do
                            if not(iStartPoint == aiBrain.M27StartPositionNumber) and M27Utilities.GetDistanceBetweenPositions(M27MapInfo.PlayerStartPoints[iStartPoint], M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain)) >= 30 then
                                ...
                                tSMLTarget, iSMLDamage = M27Logic.GetBestAOETarget(aiBrain, M27MapInfo.PlayerStartPoints[iStartPoint], iAOE, iDamage, true, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], 0, 5)
                                if iSMLDamage >= 27000 then iValuableTargets = iValuableTargets + 1 end
                            end
                        end
            ...
            if not(iCategoryToBuild) then
                ...
                if iFactionIndex == M27UnitInfo.refFactionUEF then
                    ...
                    if aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] >= 500 or aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious] == false or (aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithLand] == false and aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] >= 350) then
                        ...
                        if iNovaxTargetValue >= iValueWanted then
                            iCategoryToBuild = M27UnitInfo.refCategoryNovaxCentre
                            if bDebugMessages == true then LOG(sFunctionRef..': Will build a novax') end
                        end
                    end
                    if not(iCategoryToBuild) then
                        --Do we want a fatboy?
                        if bDebugMessages == true then LOG(sFunctionRef..': Deciding if we want a fatboy or a T3 arti; aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithLand]='..tostring(aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithLand])..'; aiBrain[M27Overseer.refiDistanceToNearestEnemyBase]='..aiBrain[M27Overseer.refiDistanceToNearestEnemyBase]..'; aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryLandExperimental)='..aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryLandExperimental)..'; Enemy T2 arti='..table.getn(aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryFixedT2Arti, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], 725, 'Enemy'))..'; M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryLandExperimental, 3)='..tostring(M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryLandExperimental, 3))) end
                        if aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithLand] and aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] <= 600 and aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryLandExperimental) <= 1 and table.getn(aiBrain:GetUnitsAroundPoint(M27UnitInfo.refCategoryFixedT2Arti, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber], 725, 'Enemy')) <= 2 and (M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryLandExperimental, 3) or not(M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryFixedT3Arti, 1))) then
                            iCategoryToBuild = M27UnitInfo.refCategoryLandExperimental
                        else
                            ...
                            if ((not(aiBrain[M27MapInfo.refbCanPathToEnemyBaseWithAmphibious]) and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 10) or (aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 15 and (aiBrain[M27Overseer.refiDistanceToNearestEnemyBase] >= 325 or not(M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryLandExperimental, 4))))) and aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryT3Mex) >= math.max(1, table.getn(M27MapInfo.tResourceNearStart[M27Utilities.GetAIBrainArmyNumber(aiBrain)][1])) then
                                iCategoryToBuild = M27UnitInfo.refCategoryFixedT3Arti
                            end
                        end
                    end
                else
                    --Other faction logic - for now placeholder for T3 arti
                    local iCurrentT3Arti = aiBrain:GetCurrentUnits(M27UnitInfo.refCategoryFixedT3Arti)
                    if not(M27Conditions.LifetimeBuildCountLessThan(aiBrain, M27UnitInfo.refCategoryLandExperimental, 3 + iCurrentT3Arti)) and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 12 then
                        iCategoryToBuild = M27UnitInfo.refCategoryFixedT3Arti
                    end
                end
    ...
    if M27Utilities.IsTableEmpty(ScenarioInfo.Options.RestrictedCategories) then
        ...
        if M27Utilities.IsTableEmpty(tT3EngisOfOurFaction) == false then
            ...
            if not(M27FactoryOverseer.GetBlueprintsThatCanBuildOfCategory(aiBrain, iCategoryToBuild, oT3EngiOfOurFaction)) then
                --Cant build the desired experimental, try every experimental type category
                local tPossibleCategory = {M27UnitInfo.refCategoryFixedT3Arti, M27UnitInfo.refCategoryLandExperimental, M27UnitInfo.refCategoryAllAir * categories.EXPERIMENTAL, M27UnitInfo.refCategoryNovaxCentre, M27UnitInfo.refCategorySML, M27UnitInfo.refCategoryExperimentalStructure}
                for _, iAltCategory in tPossibleCategory do
                    if M27FactoryOverseer.GetBlueprintsThatCanBuildOfCategory(aiBrain, iCategoryToBuild, oT3EngiOfOurFaction) then
                        iCategoryToBuild = iAltCategory
                        break
                    end
                end
                if bDebugMessages == true then LOG(sFunctionRef..': Cant build chosen category so will try all other experimental categories rather than give up on building experimental altogether') end
            end
        end
    end
...
function GetCategoryToBuildFromAction(iActionToAssign, iMinTechLevel, aiBrain)
    ...
    elseif iActionToAssign == refActionBuildShield then
        --NOTE: Separately this gets changed to tech3
...
function UpgradeBuildingActionCompleteChecker(aiBrain, oEngineer, oBuildingToUpgrade)
    ...
    while bContinue == true do
        ...
        if bDebugMessages == true then
            ...
            else LOG('Unit is valid; building to upgrade='..oBuildingToUpgrade:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oBuildingToUpgrade)..'; Building state='..M27Logic.GetUnitState(oBuildingToUpgrade)) end
        end
...
function AttackMoveToRandomPositionAroundBase(aiBrain, oEngineer, iMaxDistance, iMinDistance)
    ...
    if tRandomTargetLocation == nil then
        ...
        if M27MapInfo.RecheckPathingOfLocation(sPathing, oEngineer, tActionTargetLocation, nil) then
            ...
            LOG(sFunctionRef..': aiBrain with index='..aiBrain:GetArmyIndex()..' failed to get random location that can path to; oEngineer='..oEngineer:GetUnitId()..GetEngineerUniqueCount(oEngineer)..'; iEngiPathingGroup='..iEngiPathingGroup..'; Pathing group of base='..M27MapInfo.GetSegmentGroupOfLocation(sPathing, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber]))
        end
    end
...
function GetLikelySMLTarget(aiBrain, oSML)
    ...
    local tLikelyTarget, iBestDamage = M27Logic.GetBestAOETarget(aiBrain, M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain), iAOE, iDamage, true, oSML:GetPosition(), 0, 5)
    ...
    if iBestDamage < 27000 then
        for iStartPoint = 1, table.getn(M27MapInfo.PlayerStartPoints) do
            if not(iStartPoint == aiBrain.M27StartPositionNumber) and M27Utilities.GetDistanceBetweenPositions(M27MapInfo.PlayerStartPoints[iStartPoint], M27MapInfo.GetPrimaryEnemyBaseLocation(aiBrain)) >= 30 then
                if bDebugMessages == true then LOG(sFunctionRef..': Considering the start position '..iStartPoint..'='..repr(M27MapInfo.PlayerStartPoints[iStartPoint])) end
                tSMLTarget, iSMLDamage = M27Logic.GetBestAOETarget(aiBrain, M27MapInfo.PlayerStartPoints[iStartPoint], iAOE, iDamage, true, oSML:GetPosition(), 0, 5)
...
function CheckForEnemySMD(aiBrain, oSML)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': Starting loop for checking for enemy SMD; Nuke details='..oSML:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oSML)) end
        ...
        if M27Utilities.IsTableEmpty(tEnemySMD) == false then
            for iSMD, oSMD in tEnemySMD do
                ...
                if not(bAlreadyConsideredSMD) then
                    ...
                    if bDebugMessages == true then
                        LOG(sFunctionRef..': Considering oSMD for the first time, oSMD='..oSMD:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oSMD)..'; Enemy SMD fraction complete='..oSMD:GetFractionComplete()..'; SML fraction complete='..oSML:GetFractionComplete())
    ...
    if oEngineer then
        if M27UnitInfo.IsUnitValid(oEngineer) then
            if bDebugMessages == true then
                ...
                if oActionTargetObject then LOG('oActionTargetObject='..oActionTargetObject:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oActionTargetObject))
            ...
            if iActionToAssign == refActionSpare then
                ...
                if iActionToAssign == refActionReclaimArea or iActionToAssign == refActionReclaimTrees then
                    ...
                    if oActionTargetObject then
                        if not(iActionToAssign == refActionReclaimArea) and not(iActionToAssign == refActionReclaimTrees) and not(iActionToAssign == refActionBuildShield and not(EntityCategoryContains(categories.MOBILE, oActionTargetObject:GetUnitId()))) then
                    ...
                    if bAreAssisting == false then
                        ...
                        if bConstructBuilding == true then
                            ...
                            if bQueueUpMultiple and (iActionToAssign == refActionBuildT3ArtiPower) then
                                ...
                                tTargetLocation = BuildStructureAtLocation(aiBrain, oEngineer, iCategoryToBuild, iMaxAreaToSearch, iCatToBuildBy, tTargetLocation, nil, nil, oUnitToBuildBy)
                                ...
                                if M27Utilities.IsTableEmpty(tTargetLocation) == true then
                                    ...
                                    if bDebugMessages == true then
                                        LOG(sFunctionRef..': Will give results for if we have flagged anywhere in a 3x3 grid centred on target location with the same action (whether this is correct will depend on the building size')
                                        local sLocationRef
                                        for iAdjX = -1, 1, 1 do
                                            for iAdjZ = -1, 1, 1 do
                                                sLocationRef = M27Utilities.ConvertLocationToReference({tTargetLocation[1] + iAdjX, tTargetLocation[2], tTargetLocation[3] + iAdjZ})
                            ...
                            if bQueueUpMultiple == true and M27Utilities.IsTableEmpty(tTargetLocation) == false then
                                ...
                                elseif iActionToAssign == refActionBuildT3ArtiPower then
                                    ...
                                    for iUnit, oUnit in aiBrain:GetListOfUnits(M27UnitInfo.refCategoryFixedT3Arti, false, false) do
                                        if bDebugMessages == true then LOG(sFunctionRef..': Considering adjacency locations for T3 Arti='..oUnit:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oUnit)) end
                                        if M27Utilities.IsTableEmpty(oUnit[M27UnitInfo.reftAdjacencyPGensWanted]) == false then
                                            for iSubtable, tSubtable in oUnit[M27UnitInfo.reftAdjacencyPGensWanted] do
                                                if bDebugMessages == true then LOG(sFunctionRef..': Arti='..oUnit:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oUnit)..'; iSubtable='..iSubtable..'; tSubtable='..repr(tSubtable)) end
                            ...
                            elseif iActionToAssign == refActionBuildT3MexOverT2 then
                                ...
                                if M27FactoryOverseer.GetBlueprintsThatCanBuildOfCategory(aiBrain, M27UnitInfo.refCategoryT3Mex, oEngineer) then
                                    ...
                                    M27Utilities.ErrorHandler('Unable to build a T3 mex with engineer '..oEngineer:GetUnitId()..GetEngineerUniqueCount(oEngineer)..'; either unit restrictions are active or something has gone wrong with the code', nil, true)
                                    aiBrain[refiTimeOfLastFailure][iActionToAssign] = GetGameTimeSeconds()
                                    IssueSpareEngineerAction(aiBrain, oEngineer)
                                    UpdateEngineerActionTrackers(aiBrain, oEngineer, iActionToAssign, nil, false, iConditionNumber)
                                end
...
function GetActionTargetAndObject(aiBrain, iActionRefToAssign, tExistingLocationsToPickFrom, tIdleEngineers, iActionPriority, tsUnitStatesToIgnoreCurrent, iSearchRangeForPrevEngi, iSearchRangeForNearestEngi, bOnlyReassignIdle, bGetInitialEngineer, iMinTechLevelWanted)
    ...
    if bDebugMessages == true then
        ...
        else LOG(sFunctionRef..': oActionObject='..oActionObject:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oActionObject))
        end
    end
    M27Utilities.FunctionProfiler(sFunctionRef, M27Utilities.refProfilerEnd)
    ...
    if M27Utilities.IsTableEmpty(aiBrain[reftUnitsWithFixedShield]) == false then
        ...
        while bChangesMade do
            ...
            for iUnit, oUnit in aiBrain[reftUnitsWithFixedShield] do
                if not(M27UnitInfo.IsUnitValid(oUnit)) or not(M27Logic.IsTargetUnderShield(aiBrain, oUnit, 3000, false, true, true)) then
                    if M27UnitInfo.IsUnitValid(oUnit) then
                        table.insert(aiBrain[reftUnitsWantingFixedShield])
                        oUnit[refbNeedsLargeShield] = false
                    end
...
function RefreshUnitsWantingFixedShields(aiBrain)
    ...
    if M27Utilities.IsTableEmpty(aiBrain[reftUnitsWantingFixedShield]) == false then
        while bChangesMade do
            ...
            for iUnit, oUnit in aiBrain[reftUnitsWantingFixedShield] do
                if bDebugMessages == true then LOG(sFunctionRef..': iUnit='..iUnit..'; oUnit='..oUnit:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oUnit)..'; M27UnitInfo.IsUnitValid(oUnit)='..tostring(M27UnitInfo.IsUnitValid(oUnit))..'; Is under shield='..tostring(M27Logic.IsTargetUnderShield(aiBrain, oUnit, 3000, false, true, true))) end
                if not(M27UnitInfo.IsUnitValid(oUnit)) or M27Logic.IsTargetUnderShield(aiBrain, oUnit, 3000, false, true, true) then
                    bChangesMade = true
                    if M27UnitInfo.IsUnitValid(oUnit) then
                        --Add to table of units that have a shield
                        table.insert(aiBrain[reftUnitsWithFixedShield], oUnit)
                    end
                    table.remove(aiBrain[reftUnitsWantingFixedShield], iUnit)
                    if bDebugMessages == true then LOG(sFunctionRef..': iUnit='..iUnit..'oUnit='..oUnit:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oUnit)..': is either invalid or under shield so will remove') end
                    break
                end
            end
    ...
    if M27Utilities.IsTableEmpty(aiBrain[reftEngineerAssignmentsByActionRef]) == false then
        for iActionRef, oRecordedEngineer in aiBrain[reftEngineerAssignmentsByActionRef] do
            ...
            if M27Utilities.IsTableEmpty(aiBrain[M27EconomyOverseer.reftActiveHQUpgrades]) == false and M27Utilities.IsTableEmpty(EntityCategoryFilterDown(M27UnitInfo.refCategoryAirFactory * categories.TECH2, aiBrain[M27EconomyOverseer.reftActiveHQUpgrades])) == false and aiBrain[M27Overseer.refiOurHighestFactoryTechLevel] == 2 then
                iPowerWantedPerTick = iPowerWantedPerTick + 50
            end
            ...
            while iEngineersToConsider >= 0 do --want >= rather than > so get correct calculation of engineers needed
                ...
                if aiBrain[M27Overseer.refiAIBrainCurrentStrategy] == M27Overseer.refStrategyACUKill and aiBrain[M27Overseer.refbIncludeACUInAllOutAttack] then
                    ...
                    elseif iCurrentConditionToTry == 8 then --Low power action part 1; also T3 power adjacency logic (as want this built as high priority  ahead of normal power)
                        ...
                        if M27Utilities.IsTableEmpty(tT3Arti) == false then
                            ...
                            if M27Utilities.IsTableEmpty(aiBrain[reftEngineerAssignmentsByActionRef][refActionBuildT3ArtiPower]) == false then iActionToAssign = refActionBuildT3ArtiPower
                                ...
                                for iT3Arti, oT3Arti in tT3Arti do
                                    ...
                                    if bDebugMessages == true then LOG(sFunctionRef..': Just called logic to refresh T3 arti adjacency locations for T3Arti='..oT3Arti:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oT3Arti)..'; Is the table of PGens wanted empty='..tostring(M27Utilities.IsTableEmpty(oT3Arti[M27UnitInfo.reftAdjacencyPGensWanted]))) end
                        ...
                        if not(iActionToAssign) then --Normal low power action
                            ...
                            if iMassStoredRatio == nil then iMassStoredRatio = aiBrain:GetEconomyStoredRatio('MASS') end
                            if bHaveLowPower or (iMassStoredRatio >= 0.05 and bWantMorePower and aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] <= 40) or (aiBrain[M27EconomyOverseer.refiEnergyGrossBaseIncome] <= 275 and M27Utilities.IsTableEmpty(aiBrain[M27EconomyOverseer.reftActiveHQUpgrades]) == false and M27Utilities.IsTableEmpty(EntityCategoryFilterDown(M27UnitInfo.refCategoryAirFactory * categories.TECH2, aiBrain[M27EconomyOverseer.reftActiveHQUpgrades])) == false) then
                    ...
                    elseif iCurrentConditionToTry == 9 then --High priority energy storage
                        --Want to build energy storage if dont have any, and either have no ACU, or ACU is in combat and have high energy
                        if aiBrain[M27EconomyOverseer.refiEnergyNetBaseIncome] >= 1 then
                            if M27Utilities.IsACU(M27Utilities.GetACU(aiBrain)) then
                                --Does the ACU have more than 2 enemy units near it?
                                if (M27Utilities.GetACU(aiBrain).PlatoonHandle[M27PlatoonUtilities.refiEnemiesInRange] or 0) >= 3 then
                                    if iEnergyStoredRatio == nil then iEnergyStoredRatio = aiBrain:GetEconomyStoredRatio('ENERGY') end
                    ...
                    elseif iCurrentConditionToTry == 10 then --Initial land factories (high priority with low resource conditions)
                        if iLandFactories == nil then
                            iLandFactories = aiBrain:GetCurrentUnits(refCategoryLandFactory)
                            if iLandFactories == nil then iLandFactories = 0 end
                        end
                    ...
                    elseif iCurrentConditionToTry == 11 then --SMD
                    ...
                    elseif iCurrentConditionToTry == 12 then --Nuke
                    ...
                    elseif iCurrentConditionToTry == 13 then --Unclaimed mexes within defender coverage
                    ...
                    elseif iCurrentConditionToTry == 14 then --Hydro within our defence coverage?'
                    ...
                    elseif iCurrentConditionToTry == 15 then --Lower power action part 2 - want enough power to support guncom
                    ...
                    elseif iCurrentConditionToTry == 16 then --Mass storage around T2/T3 mexes
                    ...
                    elseif iCurrentConditionToTry == 17 then --T3 mexes in place of existing T2 mexes
                        if iHighestFactoryOrEngineerTechAvailable >= 3 then
                    ...
                    elseif iCurrentConditionToTry == 18 then --Queue up an experimental if we have significant mass as it may take time to find the right position
                        if iMassStored == nil then iMassStored = aiBrain:GetEconomyStored('MASS') end
                        if not(bHaveLowPower) and not(bHaveLowMass) and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 7 and iMassStored >= 1000 and aiBrain[M27EconomyOverseer.refiMassNetBaseIncome] > 1 and iHighestFactoryOrEngineerTechAvailable >= 3 and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 7 and (not(M27Conditions.LifetimeBuildCountLessThan(aiBrain, categories.EXPERIMENTAL + M27UnitInfo.refCategoryFixedT3Arti + M27UnitInfo.refCategorySML, 1)) or (aiBrain[M27EconomyOverseer.refiMassNetBaseIncome] > 10 and aiBrain[M27EconomyOverseer.refiMassGrossBaseIncome] >= 14)) then
                            ...
                            iSearchRangeForNearestEngi = 70 --Dont want too far away as if in high mass scenario could end up overflowing by the time the engi arrives
                            iMaxEngisWanted = 1
                    ...
                    elseif iCurrentConditionToTry == 19 then --Build factories (or upgrade HQ) if getting too much mass
                    ...
                    elseif iCurrentConditionToTry == 20 then --Air staging if we need one for low fuel air units
                        ...
                        --end
                    elseif iCurrentConditionToTry == 21 then --Energy storage once have certain level of power
                        if bHaveLowPower == false then
                    ...
                    elseif iCurrentConditionToTry == 22 then --Get reclaim
                    ...
                    elseif iCurrentConditionToTry == 24 then --Try to get nearest unclaimed mex (i.e. this will only run if are no mexes within defensive area or our side of map):
                        if iAllUnclaimedMexesInPathingGroup == nil then
                            tAllUnclaimedMexesInPathingGroup = GetUnclaimedMexes(aiBrain, sPathing, iPathingGroup, false, false, false)
                            if M27Utilities.IsTableEmpty(tAllUnclaimedMexesInPathingGroup) == false then iAllUnclaimedMexesInPathingGroup = table.getn(tAllUnclaimedMexesInPathingGroup)
                            else iAllUnclaimedMexesInPathingGroup = 0 end
                    ...
                    elseif iCurrentConditionToTry == 25 then --2nd T1 power construction with low priority engineers
                    ...
                    elseif iCurrentConditionToTry == 26 then --Radar near base
                        if bHaveLowPower == false then
                    ...
                    elseif iCurrentConditionToTry == 27 then --Shields
                        if bHaveLowPower == false and bHaveLowMass == false and iHighestFactoryOrEngineerTechAvailable >= 2 and M27Utilities.IsTableEmpty(aiBrain[reftUnitsWantingFixedShield]) == false then
                    ...
                    elseif iCurrentConditionToTry == 28 then --reclaim on enemy side of map
                    ...
                    elseif iCurrentConditionToTry == 29 then --Assist air
                    ...
                    elseif iCurrentConditionToTry == 30 then --Sonar
                    ...
                    elseif iCurrentConditionToTry == 31 then --More reclaim (lower priority locations)
                        if iMassStoredRatio == nil then iMassStoredRatio = aiBrain:GetEconomyStoredRatio('MASS') end
                    ...
                    elseif iCurrentConditionToTry == 32 then --Experimental if have loads of mass or satisfy other tests; one of these will be if we ahve already started on an experimental
                        if iHighestFactoryOrEngineerTechAvailable >= 3 then --at least 250 gross income ignoring reclaim
                            ...
                            if not(iActionToAssign) and M27Utilities.IsTableEmpty(aiBrain[reftEngineerAssignmentsByActionRef][refActionBuildExperimental]) == false then
                                        ...
                                        for iRef, tEngSubtable in  aiBrain[reftEngineerAssignmentsByActionRef][iActionToAssign] do
                                            ...
                                            if tEngSubtable[refEngineerAssignmentEngineerRef][refbPrimaryBuilder] then
                                                if M27UnitInfo.GetUnitTechLevel(tEngSubtable[refEngineerAssignmentEngineerRef]) < iHighestTechLevelEngi and not(tEngSubtable[refEngineerAssignmentEngineerRef]:IsUnitState('Building')) then
                                                    ...
                                                    ClearEngineerActionTrackers(aiBrain, tEngSubtable[refEngineerAssignmentEngineerRef], false)
                                                end
                                                break
                                            end
                                        end
                        ...
                        if iActionToAssign and iExistingEngineersAssigned <= iMaxEngisWanted then
                            ...
                            if iExistingEngineersAssigned < iMaxEngisWanted then
                                ...
                                if iEngineersToConsider > 0 then
                                    ...
                                    if bDebugMessages == true then
                                        ...
                                        else LOG('ActionTargetObject='..oActionTargetObject:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(oActionTargetObject))
                                        end
                                    end
                                end
                                ...
                                if M27Utilities.IsTableEmpty(tActionTargetLocation) == true and oActionTargetObject == nil then
                                    ...
                                    if bDebugMessages == true then LOG(sFunctionRef..': Have no action target or object target, so unless an error message has appeared we are just calculating how many engis we want to build') end
                                else
                                    ...
                                    if oEngineerToAssign == nil and iEngineersToConsider > 0 then oEngineerToAssign = GetNearestEngineerWithLowerPriority(aiBrain, tIdleEngineers, iCurrentConditionToTry, tActionTargetLocation, iActionToAssign, tsUnitStatesToIgnoreCurrent, iSearchRangeForPrevEngi, iSearchRangeForNearestEngi, bOnlyReassignIdle, bGetInitialEngineer, iMinEngiTechLevelWanted) end
                                    if oEngineerToAssign then
                                        if oEngineerToAssign.GetPlan then M27Utilities.ErrorHandler('oEngineer is a platoon, plan='..oEngineer:GetPlan()..(oEngineer[M27PlatoonUtilities.refiPlatoonCount] or 'nil')) end
                                        if bDebugMessages == true then
                            ...
                            if bDebugMessages == true then
                                ...
                                for iRef, tRef in aiBrain[reftEngineerAssignmentsByActionRef][iActionToAssign] do
                                    LOG('iRef='..iRef..'; Engineer ID and LC='..tRef[refEngineerAssignmentEngineerRef]:GetUnitId()..M27UnitInfo.GetUnitLifetimeCount(tRef[refEngineerAssignmentEngineerRef]))
                                end
                            end
                        end
                    end
...
function CheckAllEngineerLocations(aiBrain)
    ...
    if iOldLocations > 0 then
        M27Utilities.ErrorHandler('Have old locations, will list out before clearing; 1st val=location, 2nd=action, 3rd=engiref: '..repr(tOldLocations))
