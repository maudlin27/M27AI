...
refiPlatoonMaxRange = 'M27PlatoonMaxRange' --Higher of direct and indirect fire range for the platoon
        ...
        if table.getn(tNearbyEnemies) == 0 then bIsAwayFromNearbyEnemies = true
            ...
            if table.getn(tEnemiesWithThreat) == 0 then
                ...
                if M27Utilities.GetDistanceBetweenPositions(tCurPos, tCurDestination) <= iComparisonDistance then
                    ...
                    if bDebugMessages == true then
                        LOG(sFunctionRef..': Only want to move 5 along the current destination to see if its further from enemy; tRunAwayComparisonPos='..repru(tRunAwayComparisonPos))
...
function UpdatePlatoonActionForNearbyEnemies(oPlatoon, bAlreadyHaveAttackActionFromOverseer)
    ...
    if not(sPlatoonName == 'M27MobileShield') then
        ...
        elseif sPlatoonName == 'M27Skirmisher' then
        ...
        if iNearbyEnemies > 0 then
                            ...
                            if oPlatoon[refiEnemiesInRange] > 0 then
                                ...
                                if oPlatoon[refbKiteEnemies] == true then
                                    if oPlatoon[refbACUInPlatoon] == true or oPlatoon[refiDFUnits] > 0 or (oPlatoon[refiIndirectUnits] > 0 and EntityCategoryContains(categories.EXPERIMENTAL, oPlatoon[refoFrontUnit])) then
                                        ...
                                        local iEnemyMaxRange = 18 --Assumed
                                        if oNearestEnemy or oNearestPD then
                                            ...
                                            if oPlatoon[refiPlatoonMaxRange] >= iEnemyMaxRange then --if have same max range may still be benefit to kiting if enemy lacks intel
                                                if oNearestEnemy == oNearestPD then
                                                    ...
                                                    if iNearestEnemyDistance > oPlatoon[refiPlatoonMaxRange] then
                                                        ...
                                                        if not(sPlatoonName == 'M27Skirmisher') or M27Logic.GetIntelCoverageOfPosition(aiBrain, GetPlatoonFrontPosition(oPlatoon), math.max(oPlatoon[refiPlatoonMaxRange] - 5, math.min(iNearestEnemyDistance, oPlatoon[refiPlatoonMaxRange])), false) then
                                                            bDontConsiderFurtherOrders = true
                                                            oPlatoon[refiCurrentAction] = refActionMoveDFToNearestEnemy
                                                            ...
                                                            --Are a skirmisher platoon that lacks intel so fall back
                                                            bDontConsiderFurtherOrders = true
                                                            oPlatoon[refiCurrentAction] = refActionTemporaryRetreat
                                                            if bDebugMessages == true then LOG(sFunctionRef..': Skirmisher platoon that lacks sufficient intel so will fall back') end
                                                        ...
                                                        if iIntelCoverage >= math.max(iEnemyMaxRange, 22) then --ACU has range of 22 (no gun) and vision of 26, so want to have intel coverage of at least this before consider other actions
                                                            ...
                                                            if sPlatoonName == 'M27Skirmisher' then iDistToConsiderIfTheyAreRunningAway = 0 end
                                                            ...
                                                            if iDistanceInsideOurRange > iDistToConsiderIfTheyAreRunningAway then
                                                                ...
                                                                if iPrevDistanceInsideOurRange == nil or iPrevDistanceInsideOurRange < iDistanceInsideOurRange then --Enemy getting close
                                                                    ...
                                                                    if sPlatoonName == 'M27Skirmisher' and iIntelCoverage < math.min(40, oPlatoon[refiPlatoonMaxRange]) then
                                            ...
                                            if oPlatoon[refiCurrentAction] == refActionKitingRetreat and M27UnitInfo.IsUnitValid(oNearestEnemy) and not(sPlatoonName == 'M27Skirmisher') then
                                    ...
                                    if sPlatoonName == 'M27EscortAI' then
                                        ...
                                        if oPlatoon[refiEnemyStructuresInRange] > 0 and M27Logic.GetCombatThreatRating(aiBrain, oPlatoon[reftEnemyStructuresInRange], false) > 100 then
                                            ...
                                            if oPlatoon[refoPlatoonOrUnitToEscort] then
                                                ...
                                                if iNearestEnemyUnit <= 25 then
                                                    ...
                                                    if iNearestEnemyUnit <= 35 then
                                                        ...
                                                        if bDebugMessages == true then LOG(sFunctionRef..': Nearest enemy unit is within 30 so moving to the front of the platoon='..repru(tPlatoonToEscortPosition)) end
                                                    else
                                                        ...
                                                        if bDebugMessages == true then LOG(sFunctionRef..': Nearest enemy unit is more than 35 from escorted platoon so will get a position behind the escorted platoon; tPlatoonToEscortPosition='..repru(tPlatoonToEscortPosition)..'; position to move to='..repru(oPlatoon[reftTemporaryMoveTarget])) end
                                                    end
                                                end
                                            else
                                                oPlatoon[refiCurrentAction] = refActionTemporaryRetreat
                                            ...
                                            if oPlatoon[refoPlatoonOrUnitToEscort][refiEnemiesInRange] > 0 then
                                                ...
                                                if M27Utilities.IsTableEmpty(tNearbyACU) then
                                                    if bDebugMessages == true then LOG(sFunctionRef..': Our ACU is nearby so wont attack') end
                                                    oPlatoon[refiCurrentAction] = refActionAttack
                                                end
                                            else
            ...
            if sPlatoonName == 'M27Skirmisher' then
        ...
        if M27Utilities.IsTableEmpty(oPlatoon[reftCurrentUnits]) == true then
            ...
            if bUpdateByUnitType == true then
                ...
                if not(bACUInPlatoon) then
                    ...
                    if oPlatoon[refiIndirectUnits] > 0 then
                        if oPlatoon[refiDFUnits] == 0 then oPlatoon[refiPlatoonMaxRange] = math.max(oPlatoon[refiPlatoonMaxRange], M27UnitInfo.GetUnitIndirectRange(oPlatoon[refoFrontUnit]))
                        else oPlatoon[refiPlatoonMaxRange] = math.max(oPlatoon[refiPlatoonMaxRange], M27UnitInfo.GetUnitIndirectRange(M27Utilities.GetNearestUnit(oPlatoon[reftIndirectUnits], GetPlatoonFrontPosition(oPlatoon), aiBrain)))
                        end
                    end
                end
                ...
                if iDistToFirebase <= 30 or (iDistToFirebase <= 60 and oPlatoon[refiEnemiesInRange] >= 6 and M27Utilities.GetDistanceBetweenPositions(oACU:GetPosition(), M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber]) < M27Utilities.GetDistanceBetweenPositions(tFirebasePosition, M27MapInfo.PlayerStartPoints[aiBrain.M27StartPositionNumber])) then
                    ...
                    if bReclaimInstead then
                        ...
                        if (oACU:IsUnitState('Building') or oACU:IsUnitState('Repairing')) and M27UnitInfo.IsUnitValid(oACU:GetFocusUnit()) and oACU:GetFocusUnit():GetFractionComplete() >= 0.25 and EntityCategoryContains(categories.SHIELD, oACU:GetFocusUnit()) then
                            ...
                            if oUnderConstructionPD or (iNearbyT2PlusPD <= math.min(15, math.max(4, aiBrain[M27Overseer.refiTotalEnemyShortRangeThreat] / 400)) and (iNearbyT2PlusPD < 4 or iNearestEnemy <= 150) and (iNearbyT2PlusPD < 2 or (iNearestEnemy <= 45 and iDistToFirebase <= 30) or (iNearbyT2PlusPD <= 7 and iNearestEnemy <=55) or (iNearbyT2PlusPD < 6 and iT2PDNearToEnemy <= 0 and iNearestEnemy <= 80 and aiBrain[M27Overseer.refiTotalEnemyShortRangeThreat] >= 3500) or (iT2PDNearToEnemy <= 1 and M27Conditions.HaveLowMass(aiBrain) == false and iNearestEnemy <= 80 and aiBrain[M27Overseer.refiTotalEnemyShortRangeThreat] >= 3500) or not(iNearbyFirebaseRef))) then
...
function RetreatToMobileShields(oPlatoon)
    ...
    if oPlatoon[refoSupportingShieldPlatoon] and oPlatoon[refoSupportingShieldPlatoon][refiUnitsWithShields] > 0 and aiBrain:PlatoonExists(oPlatoon[refoSupportingShieldPlatoon]) then
        ...
        if iDistanceToShieldHelper > 50 and iDistanceToShieldHelper <= 250 and not(oPlatoon[refoSupportingShieldPlatoon]:GetPlan() == 'M27RetreatingShieldUnits') then
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Just got merge location; result='..repru(oPlatoon[reftTemporaryMoveTarget] or {'nil'})) end
    ...
    if oPlatoon and oPlatoon.GetBrain then
        ...
        if aiBrain and aiBrain.PlatoonExists and aiBrain:PlatoonExists(oPlatoon) then
                    ...
                    if not(bDoNothing) then
                        ...
                        if not(oPlatoon[refiCurrentAction] == refActionDisband) then
                            ...
                            if not(oPlatoon[refiPlatoonMaxRange]) then
                                ...
                                if oPlatoon[refiCurrentUnits] > 0 then oPlatoon[refiPlatoonMaxRange] = math.max(2, GetUnitMaxGroundRange(oPlatoon[reftCurrentUnits]))
                            ...
                            if aiBrain[M27Overseer.refiEnemyHighestTechLevel] >= 3 then iEnemySearchRadius = math.max(iEnemySearchRadius, 60) end
            ...
            if oPlatoon[reftPrevAction][1] == oPlatoon[refiCurrentAction] then
                ...
                if bDebugMessages == true then LOG('Prev action is different to current action, so will refresh unless is a movement path special case or assisting building when already assisting; bRefreshAction='..tostring(bRefreshAction)..'; iRefreshActionThreshold='..iRefreshActionThreshold) end
            end
            if bRefreshAction == true then
                ...
                if (oPlatoon[refiRefreshActionCount] or 0) < iRefreshActionThreshold then bRefreshAction = false end
                        ...
                        if bHaveUnitToFollow then
                            ...
                            if M27UnitInfo.IsUnitValid(oPlatoon[refoFrontUnit]) then
                                ...
                                if bDebugMessages == true then
                                    ...
                                    LOG(sFunctionRef..': tNewTargetPath='..repru(tNewTargetPath or {'nil'})..'; tTargetUnitPosition='..repru(tTargetUnitPosition))
                                end
                                if M27Utilities.IsTableEmpty(tNewTargetPath) == true then
                                    ...
                                    if bDebugMessages == true then LOG(sFunctionRef..': tNewTargetPath='..repru(tNewTargetPath)..'; will set the movement path equal to this') end
    ...
    if oPlatoon and oPlatoon.GetBrain then
        ...
        if aiBrain and aiBrain.PlatoonExists and aiBrain:PlatoonExists(oPlatoon) then
            ...
            if oPlatoon[refiCurrentAction] == nil then
                ...
                local bPlatoonNameDisplay = false
                if bDebugMessages == true then
                    LOG(sFunctionRef..': ACU state='..M27Logic.GetUnitState(M27Utilities.GetACU(aiBrain))..'; Special Micro='..tostring(M27Utilities.GetACU(aiBrain).M27UnitInfo.refbSpecialMicroActive or false)..'; ACU movementpath='..repru((oPlatoon[reftMovementPath] or {'nil'}))..'; Previously run='..tostring(oPlatoon[refbHavePreviouslyRun]))
                    ...
                    if oPlatoon[refiEnemiesInRange] == 0 and oPlatoon[refiEnemyStructuresInRange] == 0 then
                        ...
                        elseif sPlatoonName == 'M27GroundExperimental' then
                            ...
                            if bHaveFatboy and oPlatoon[refiEnemyStructuresInRange] > 0 then
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': Dealing with an experimental platoon, will see if its near enemy ACU by looking for enemies based on oPlatoon[refiPlatoonMaxRange]='..oPlatoon[refiPlatoonMaxRange]..'; oPlatoon[refiIndirectUnits]='..oPlatoon[refiIndirectUnits]..'; oPlatoon[refiDFUnits]='..oPlatoon[refiDFUnits]) end
                            ...
                            if M27Utilities.IsTableEmpty(tDFTargetPosition) and not(oUnitToAttackInstead) then
                                ...
                                if M27Utilities.IsTableEmpty(tDFTargetPosition) and not(oUnitToAttackInstead) and not(oNearestUnitOfInterest) then
                                    --Nearby T3 land?
                                    if oPlatoon[refiEnemiesInRange] > 0 then
                                        tNearbyEnemiesOfInterest = EntityCategoryFilterDown(M27UnitInfo.refCategoryMobileLand * categories.TECH3 * categories.DIRECTFIRE + M27UnitInfo.refCategoryMobileLand * categories.TECH3 * categories.INDIRECTFIRE, oPlatoon[reftEnemiesInRange])
                                        if M27Utilities.IsTableEmpty(tNearbyEnemiesOfInterest) == false then
                                    ...
                                    if M27Utilities.IsTableEmpty(tDFTargetPosition) and not(oUnitToAttackInstead) and not(oNearestUnitOfInterest) then
                                        if oPlatoon[refiEnemyStructuresInRange] > 0 then
                                            ...
                                            if not(M27Utilities.IsTableEmpty(tNearbyEnemiesOfInterest)) then
                                                ...
                                                if EntityCategoryContains(categories.TECH3, oNearestUnitOfInterest.UnitId) then iDistOutOfOurRange = 20 end
                ...
                elseif oPlatoon[refiCurrentAction] == refActionRun or oPlatoon[refiCurrentAction] == refActionTemporaryRetreat or oPlatoon[refiCurrentAction] == refActionKitingRetreat then
                    ...
                    if M27Utilities.IsTableEmpty(tCurrentUnits) == false then
                        ...
                        if bDebugMessages == true then LOG(sPlatoonName..oPlatoon[refiPlatoonCount]..': Start of refActionRun and refActionTemporaryRetreat; bTemporaryRetreat='..tostring(bTemporaryRetreat)..'; platoon movement path='..repru(oPlatoon[reftMovementPath])) end
                        ...
                        if oPlatoon[refiEnemiesInRange] + oPlatoon[refiEnemyStructuresInRange] == 0 and not(oPlatoon[refiCurrentAction] == refActionKitingRetreat) then
                            ...
                            if bGoToChokepoint then
                                ...
                                if bDebugMessages == true then LOG(sPlatoonName..oPlatoon[refiPlatoonCount]..': oPlatoon[reftMovementPath][oPlatoon[refiCurrentPathTarget]]='..repru(oPlatoon[reftMovementPath][oPlatoon[refiCurrentPathTarget]])) end
                        ...
                        if bDebugMessages == true then LOG(sPlatoonName..oPlatoon[refiPlatoonCount]..': Point 2: platoon movement path='..repru(oPlatoon[reftMovementPath])) end
                        ...
                        if bTargetAwayFromNearestEnemy == false then
                            ...
                            if bTargetAwayFromNearestEnemy == false then
                                ...
                                if bDebugMessages == true then LOG(sFunctionRef..': Will try going towards base instead. tTempMoveTarget='..repru(tTempMoveTarget)) end
                                if IsDestinationAwayFromNearbyEnemies(aiBrain, tCurPlatoonPos, tTempMoveTarget, math.max(oPlatoon[refiEnemySearchRadius],aiBrain[M27Overseer.refiSearchRangeForEnemyStructures]), false) == true then
                                    ...
                                    if IsDestinationAwayFromNearbyEnemies(aiBrain, tCurPlatoonPos, tTempMoveTarget, math.max(oPlatoon[refiEnemySearchRadius],aiBrain[M27Overseer.refiSearchRangeForEnemyStructures]), false) == true then
                                        ...
                                        if bDebugMessages == true then
                                            LOG(sPlatoonName..oPlatoon[refiPlatoonCount]..': Returning to base as moving away from nearest enemy doesnt move us away from other enemies; location that tried to move awawy to='..repru(tTempMoveTarget))
                ...
                elseif oPlatoon[refiCurrentAction] == refActionGoToNearestRallyPoint then
                    ...
                    oPlatoon[refiCurrentPathTarget] = 1
                    PlatoonMove(oPlatoon, oPlatoon[reftMovementPath][1])
                    if bDebugMessages == true then LOG(sFunctionRef..': Have updated '..sPlatoonName..oPlatoon[refiPlatoonCount]..' movement path to the nearest rally point='..repru(oPlatoon[reftMovementPath][1])..'; platoon front position='..repru(GetPlatoonFrontPosition(oPlatoon))) end
                ...
                elseif oPlatoon[refiCurrentAction] == refActionDisband then
                    ...
                    if oPlatoon then
                        ...
                        if sPlatoonName == 'M27Skirmisher' then aiBrain[reftSkirmisherPlatoonWantingIntel][sPlatoonName..oPlatoon[refiPlatoonCount]] = nil end
                        ...
                        if oPlatoon and aiBrain:PlatoonExists(oPlatoon) then
                            ...
                            if M27Utilities.IsTableEmpty(tRemainingUnits) == false then
                                if not(tRemainingUnits[1].PlatoonHandle) or tRemainingUnits[1].PlatoonHandle == oPlatoon then
                                    ...
                                    UpdateUnitNames(tRemainingUnits, "DisbandedReturnToBase", true)
