            ...
            if oUnitKilled.GetAIBrain then
                ...
                if instigator and not(instigator.GetLauncher and instigator:GetLauncher()) and instigator.UnitId and IsUnit(instigator) then
                    ...
                    if oKillerBrain.M27AI then
                        ...
                        --BM message if enemy thrown away experimental, provided we have decent economy (suggesting not dead) and they ahve no significant nearby threat, and have <=1 experimental or T3 arti building
                        if EntityCategoryContains(categories.EXPERIMENTAL, oUnitKilled.UnitId) and not(M27Chat.tiM27VoiceTauntByType['Killed nearby experimental']) and (oUnitKilled.VetExperience or oUnitKilled.Sync.totalMassKilled or 0) <= math.min(7500, oUnitKilled:GetBlueprint().Economy.BuildCostMass * 0.3) and M27Utilities.GetDistanceBetweenPositions(oUnitKilled:GetPosition(), M27MapInfo.PlayerStartPoints[oKillerBrain.M27StartPositionNumber]) <= 150 and M27Logic.GetCombatThreatRating(oKillerBrain, oKillerBrain:GetUnitsAroundPoint(categories.EXPERIMENTAL + categories.TECH3, oUnitKilled:GetPosition(), 150, 'Enemy')) <= 4000 and M27Logic.GetIntelCoverageOfPosition(oKillerBrain, oUnitKilled:GetPosition(), nil, true) >= 150 and (M27Utilities.IsTableEmpty(oKillerBrain[M27Overseer.reftEnemyArtiAndExpStructure]) or table.getn(oKillerBrain[M27Overseer.reftEnemyArtiAndExpStructure]) <= 1) and oKillerBrain[M27EconomyOverseer.refiGrossMassBaseIncome] >= 6 and oKillerBrain[M27EconomyOverseer.refiGrossEnergyBaseIncome] >= 250 then
